...
\cref{sec_benchmarking} covers benchmarking methodology, and \cref{sec_problem_statement} formalises the optimisation problem that \cref{ch_methods} addresses.
...
Beyond the choice of tile format, a number of transfer-layer optimisations can improve delivery performance: utilising the approximately \qty{14}{\kilo\byte} TCP slow-start window as discussed by Nathaniel~\cite{WhyYourWebsite}, edge caching~\cite{BestPracticesMap}, or HTTP/2 multiplexing.
...
The encoding strategy competition (\cref{sub_encoding_strategies}) is more accurately characterised as \emph{superoptimization}~\cite{massalinSuperoptimizerLookingShortest1987}: the encoder exhaustively evaluates all candidate strategies over the actual data and retains the smallest result, rather than using profile data to guide a heuristic decision as in classical profile-guided optimization.
The distinction matters: profile-guided optimization may make suboptimal choices; the competition is optimal over its search space.
...
With the background established, the optimisations problem addressed by this thesis can be stated formally.
...
The majority of peephole rules strictly reduce $\mu$: they either reduce $n_{\mathrm{AST}}$ (e.g.\ constant folding, boolean absorption, dead-branch elimination), keep $n_{\mathrm{AST}}$ unchanged but strictly reduce $d_{\max}$ (e.g.\ boolean flattening), or keep both unchanged but strictly reduce $n_{\mathrm{lit}}$ (e.g.\ redundant coercion removal).
...
Two rules can \emph{temporarily increase} $n_{\mathrm{AST}}$ within a single iteration.
...
The implementation therefore imposes a hard cap of \texttt{NORMALIZE\_FOLD\_FIXPOINT\_CAP\,=\,8} iterations, which is the formal termination guarantee; in practice, all styles in the benchmark corpus converge within three iterations.
Together, the bounded-below measure for the majority of rules and the hard cap guarantee that the fixpoint is reached in finitely many steps.
