...
In \cref{sec:exp_expressions} we isolate the eight expression-level passes, measuring their effect on style size, load time, and AST complexity in cumulative ablation order.
...
In \cref{sec:exp_mlt} we assess data-level optimisations, measuring how style-level dead layer elimination narrows the tile pruning advisory, how tile shaving reduces tile size, and how the encoding strategy competition delivers the shaved data to the wire.
In \cref{sec:exp_cross_style} we test generalisation by applying the full pipeline to 15 styles beyond the deep benchmarking corpus, and in \cref{sec:exp_combined} we bring all three levels together to quantify the end-to-end effect on tile volume, load time, and rendering stability - and to examine how style-level and data-level optimisations interact.
...
\noindent In this experiment we evaluate the eight expression-level passes of \cref{sub:expression_optimisations} - the rewrites that act locally on each style's JSON expressions and run to a fixpoint.

\subsection{Evaluation}\label{sub:expr_evaluation}

The first seven passes correspond to ablation steps~1-7 in the benchmark harness; we evaluate selectivity reordering separately as step~16 because it requires tile statistics.
We describe the full rendering and static-size metric set in \cref{sub:metrics}.
We report the effects on Performance in \cref{sub:cross_results,sub:generalisation_corpus}.
...
We draw several observations from the expression-level ablation.
...
We expect this: reducing an expression to a literal eliminates all per-feature evaluation cost for that expression, whereas algebraic simplifications merely shorten the evaluation path.
Fixpoint iteration amplifies the effect: a single successful fold can cascade through nested expressions, collapsing entire subtrees in subsequent iterations.
...
We note the distinction between raw, gzip, and Brotli reductions: some rewrites (e.g., \texttt{any}$\to$\texttt{in}) increase raw token count while decreasing compressed size, because the repetitive structure of \texttt{in} operand lists compresses well.
...
We acknowledge the independence assumption underlying the selectivity estimates as inaccurate for correlated map predicates (\cref{sec:query_optimisations}), and the empirical benefit may be negligible for styles whose filters are already short.
...
\noindent Where the previous experiment rewrote local expressions, in this one we evaluate the passes that reason across layers and sources - the structural family of \cref{sub:structural_optimisations}.
...
We report the effects on Performance in \cref{sub:cross_results,sub:generalisation_corpus} and the effect of the steps this work builds upon in \cref{sec:exp_expressions}.
...
We observe that raw byte count is a proxy metric for parse time and transfer cost, whereas the driver of rendering performance is layer count and per-frame draw calls; the merged layer is still cheaper to render even when its serialised form is larger.
...
We exclude symbol layers from merging because MapLibre's collision detection operates per-layer (we give the soundness argument in \cref{sub:structural_optimisations}).
...
We characterise this interaction fully in \cref{sub:mlt_discussion} and quantify it in \cref{sub:mlt_results}; the aggregate data shows that the composition is additive rather than synergistic, but the advisory-narrowing effect is a necessary precondition for style-dependent tile shaving.
...
\noindent In \cref{sec:exp_structural,sec:exp_expressions} we optimised the \emph{program} the renderer executes; in this experiment we ask whether we can optimise the \emph{data} it consumes using the same style information, and to what extent the two levels interact - the joint-optimisation question at the heart of Research Goal~R1.
The data-level pipeline we apply here (\cref{sub:data_optimisations}) takes the optimised style and uses it to drive the tile pruning advisory and the encoding strategy competition.

\textit{We carried out this experiment in collaboration with Rivian Automotive, Inc to optimise performance, correctness and software architecture of the encoder-decoder pair. We fully implemented the encoder and optimisation routine ourselves. Due to the collaboration, we do not discuss encoder performance, even though it is a bit better (taking a minute vs five hours to transcode germany) than the previous encoder/decoder pairs.}
...
In this experiment we isolate the interaction between style-level and data-level optimisations and identify which level contributes the dominant share of the end-to-end savings.
We use a cumulative design with five configurations, each adding one optimisation layer:
...
In this section we quantify the interaction between style-level and data-level optimisations previewed in \cref{sub:structural_discussion}: style-level optimisations narrow the pruning advisory, and in the following paragraphs we measure the extent of this effect before turning to the encoding evaluation.
...
At these frame rates ($>{1000}$~FPS) the renderer is largely GPU-idle; we therefore read the metric as available performance headroom on constrained devices rather than a directly observable rendering cost saving on the benchmark host.
...
We attribute the dominant bottleneck to the WASM-to-JavaScript boundary, since each decoded column must be transferred from WASM linear memory into JavaScript-visible typed arrays, incurring per-column copying and GC pressure that does not exist in a pure-native pipeline.
...
We are currently working to shift more decoding and data consumption into WASM to reduce boundary-crossing overhead.
...
\begin{figure}[H]
    ...
    \caption{Rendering metrics (load time, FPS) across configurations~1-5, enabling direct comparison with Experiments~1-2 in the combined analysis. The figure shows the significant drop in both load time and increase in FPS for tile shaving, as well as the fact that style-only optimisation is ineffective. We also confirm the \ac{MLT} implementation to sligtly decrease performance because the current implementation still internally converts to \ac{MVT}}
...
\centering
    ...
    \begin{subfigure}[t]{\linewidth}
        ...
        \ac{MLT}-Rust (this work) & \textbf{3.06} &  \textbf{2.70} &  \textbf{2.72} &  \textbf{2.75} \\
        \midrule
...
While we see this stability as consistent with the profiler capturing genuine redundancy beyond what general-purpose compressors exploit, it could equally reflect that the profiler removes the same redundancy that compressors would.
We take the stronger evidence for the profiler's value to be the absolute comparison: \ac{MLT}-Rust uncompressed (\qty{3.06}{\giga\byte}) is already smaller than \ac{MVT}+gzip (\qty{3.08}{\giga\byte}), confirming that the per-stream encoding captures type-specific redundancy that LZ77-family compressors miss on the interleaved protobuf byte stream.
This isolation directly answers the question we posed at the start of \cref{sec:exp_mlt}: the \ac{MLT}-Rust contribution is not in the columnar format alone, but in the joint strategy-selection layer that sits on top of it.

\paragraph{Scope of the encoder contribution}
We measured the \qty{28}{\percent} reduction reported in \cref{tab:mvt_mlt_comparison} on the \emph{unshaved} Germany tileset, so it is an encoder-only improvement, independent of the style-driven optimisations.
...
We regard the most significant finding of this experiment as the fact that tile shaving is the dominant contributor to load-time and transfer-size improvements, while style-level passes provide a modest, additive assist.
...
We therefore quantify the size reductions achievable \emph{without} touching geometry at all, which understates the total optimisations headroom a production pipeline could reach by composing this work with an upstream simplifier.

\section{Cross-Style Generalisation}\label{sec:exp_cross_style}
\begin{keytakeaways}
\item Static-only passes generalise: median \qty{44.5}{\percent} raw reduction across 13 unseen styles, none regressed.
\item Initial style size and complexity does not predict reduction ($r = 0.16$, $n = 13$).
\item These numbers are a lower bound - stats-driven and data-level passes were not applied here.
\end{keytakeaways}

\noindent In the preceding experiments we evaluated optimisations effectiveness on a curated corpus of 15 styles.
...
We use three tiers of breadth because end-to-end rendering benchmarks require tile data and browser automation infrastructure that does not scale to all styles: we benchmark two styles end-to-end with tile data, fifteen at the style-and-shaving level, and evaluate thirteen additional styles with static analysis only.

\subsection{Methodology}\label{sub:cross_methodology}

We use static analysis only for the cross-style evaluation: we optimise each style with all passes enabled, and compare the resulting style to the original on size and complexity metrics.
...
We observe no statistically significant correlation between initial \ac{AST} node count and reduction ($r = 0.16$, $n = 13$, $p \approx 0.6$); the sample size is too small for this correlation to be meaningful.  Americana is an outlier with the highest complexity (\num{50644} nodes) but a moderate reduction, because much of its expression complexity is in label-placement logic that the optimiser preserves.
...
We expect this: any style written by a human or generated by an editor is likely to contain at least some redundant expressions, default-valued properties, or suboptimal colour encodings.

\paragraph{Complexity and reduction}
While verbose institutional styles with several hundred layers and deeply nested expressions tend to offer more room for improvement than simple styles with fewer than 50 layers, we find that the correlation between initial style complexity and achievable reduction is not statistically significant at this sample size ($r = 0.16$, $n = 13$).
...
Because the generalisation corpus does not include tile data, we cannot apply stats-driven passes (dead elimination with geometry-type matching, stats-driven constant folding, selectivity reordering) and data-level optimisations (tile shaving, re-encoding) to the Maputnik styles.
We therefore present the reported reductions as a \emph{lower bound} on the full optimisations potential.
...
For the 11 \ac{OMT}-compatible styles in the deep corpus, we evaluated tile shaving separately against the Germany tileset (\cref{fig:tile_shave_per_style}), confirming that data-level reductions generalise across styles.
...
We observe no style in the corpus that shows a size \emph{increase} after optimisations, confirming that the passes are safe in the general case.
...
\begin{keytakeaways}
\item The three optimisation channels compose additively, not synergistically.
\item Data-level passes do almost nothing for sustained frame rate; their lever is load time.
\item No single pass dominates across styles - the multi-pass design is what generalises.
\end{keytakeaways}

\noindent In this section we present the full pipeline ablation - all 17 cumulative steps from baseline through tile rewriting - and analyse the interactions between optimisations categories.
...
We observe from the heatmap that load-time improvements are unevenly distributed across scenarios: urban, tile-dense scenarios (New York, Tokyo, Paris) show larger absolute improvements than sparse scenarios (Sahara, Amazon), because the tile-fetch component of load time dominates in dense areas.
...
As a consequence of this large improvement in frame time, previously hidden frame time consistency issues become more obvious as shown in \cref{fig:heatmapjankcount}.
We attribute these consistency issues mostly to \ac{GC}-pauses.
...
Subsequent passes produce smaller incremental reductions; we measure the total peak-heap reduction from baseline to step~15 at approximately \qty{4}{\mega\byte} (\qty{-9}{\percent}).
...
After tile shaving, we observe that expression-level constant folding and structural dead elimination tend to produce the largest marginal improvements in frame time and FPS, as they reduce the number of expressions the renderer must evaluate per feature per frame and the number of layers it must process, respectively.
...
The optimisations pipeline is an \emph{offline} stage: we optimise a style once and serve it many times, and re-encode tiles once per tileset build.
...
\Cref{tab:preprocessing_cost} reports wall-clock time for each stage of the preprocessing pipeline on the Germany OpenMapTiles tileset (\num{255694} tiles after pruning, zoom 0-14), which we measured on an AMD Ryzen~7 3700X (8 cores / 16 threads), \qty{46}{\gibi\byte} RAM, NVMe SSD; median of three runs.
...
We therefore measure the overhead ratio of the full competition at $342 / 175 = 1.95\times$ in CPU time - substantially lower than the analytical 3-5$\times$ estimate, because the cost-control mechanisms formalised in \cref{sub:preprocessing_cost_model} eliminate most redundant work.
...
The full pipeline ablation suggests three plausible channels by which optimisations improve rendering performance, which we infer from proxy metrics (AST node count, layer count, tile size) rather than direct per-mechanism instrumentation.
...
No renderer instrumentation separates these channels directly; we infer the decomposition from the proxy metrics and the time breakdown (\cref{fig:time_breakdown}).
The three channels target non-overlapping costs - simplification cannot shrink tiles and shaving cannot prune expressions - so they compose additively in the aggregate, as we already established quantitatively in \cref{sub:mlt_results}.
...
We observe diminishing marginal returns in the ablation as more passes are added.
...
Our isolated-impact analysis (non-cumulative) confirms that each pass has a non-trivial standalone contribution, even if its marginal contribution in the cumulative sequence is small.

\paragraph{Per-style variation}
We observe substantial variation in the magnitude of improvement across styles.
...
We partially self-inflict this variance: using real GPU hardware (ANGLE/Vulkan) rather than a software rasteriser captures realistic rendering costs, including GPU buffer management and shader compilation, at the expense of slightly higher measurement noise compared to deterministic software rendering.


\section{Visual Correctness Validation}\label{sec:visual_correctness}

We verified visual losslessness (\cref{sec:problem_statement}) with pixelmatch across all 15 benchmark styles at zoom levels~0-14, across all 18 scenarios, in both axis-aligned and pitched views, at \texttt{pixelRatio}~1 and using MapLibre GL~JS's Node-side software rasteriser for determinism rather than a GPU backend.
...
We conducted all rendering benchmarks on a single hardware configuration (GPU model, CPU, memory) running a specific browser version with ANGLE/Vulkan as the graphics backend.
...
We chose the 15-run median to reduce measurement noise, but rare performance events (e.g., GPU thermal throttling, garbage collection pauses) may not be fully captured.
...
We benchmark the full rendering pipeline - style optimisations through tile shaving to \ac{MLT} re-encoding - end-to-end on only 2 of the 15 styles (fiord and liberty) to reduce cost.
However, we verified the tile-shaving step alone (advisory generation and \ac{MVT} pruning, without rendering) across all 11 \ac{OMT}-compatible styles on the same Germany tileset (\cref{fig:tile_shave_per_style}), confirming that the shaving effectiveness generalises across styles with reductions from \qty{19.8}{\percent} to \qty{46.3}{\percent}.
...
We simulate network conditions via a fixed bandwidth throttle rather than measuring under real \ac{CDN} latency profiles.
We designed the synthetic camera animations (zigzag, spiral, zoomdrill, pansweep, bearingspin) to exercise diverse rendering workloads, but they may not capture typical user interaction patterns such as search-and-zoom workflows.
...
The cumulative ablation design means that the reported marginal contribution of each pass depends on the ordering of preceding passes; a leave-one-out or fractional-factorial design would isolate each pass's standalone contribution more cleanly but we did not run it for cost reasons.
...
\medskip\noindent
With the empirical evaluation complete - expression, structural, and data-level passes measured individually and in combination, generalisation tested across 15~styles, and validity threats assessed - in the following chapter we synthesise these findings, revisit the research questions posed in \cref{sec:research_goals}, and identify directions for future work.
