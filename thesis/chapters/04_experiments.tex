...
This chapter presents three experiments that evaluate the optimizations categories introduced in \cref{ch_methods}.
Expression-level optimizationss (\cref{sec_exp_expressions}) operate on the JSON representation of individual style expressions.
Structural optimizationss (\cref{sec_exp_structural}) operate on typed Rust representations of the full style document.
Data-level optimizationss (\cref{sec_exp_mlt}) transform the tile encoding itself.
The chapter concludes with a cross-style generalisation study (\cref{sec_exp_cross_style}) and a combined analysis of the full pipeline (\cref{sec_exp_combined}).

\section{Experiment 1: Expression-Level Optimisations}\label{sec_exp_expressions}

This experiment evaluates the eight expression-level passes described in \cref{sub_expression_optimizationss}: unary simplification, expression kind normalisation, constant folding, stats-driven constant folding, expression simplification, default stripping, colour minification, and selectivity reordering.
...
A smaller style reduces the time to first render because the JSON must be fetched, decompressed, and parsed before the renderer can begin.
...
In practice, most styles converge within two to three fixpoint iterations; the eight-iteration cap is never reached.
...
This experiment evaluates the structural passes described in \cref{sub_structural_optimizationss}: metadata stripping, dead layer and source elimination (with and without tile statistics), metadata refinement (from filters, paint ramps, and tile statistics), cleanup, and layer merging.
...
\begin{figure}[htbp]
    ...
    \caption{Layer count across cumulative ablation steps. Dead elimination and layer merging produce the largest reductions}
...
Paint-based refinement captures a class of optimizationss that are invisible to filter analysis - a layer whose opacity interpolates from~0 at zoom~12 is indistinguishable from a fully visible layer when examining only the filter.
...
Experiments~1 and~2 demonstrate that style-level transformations - informed by tile statistics - can significantly reduce rendering cost by simplifying the \emph{program} that the renderer executes.
This experiment addresses the complementary question: can the \emph{data} itself be transformed to reduce transfer size and decoding cost, and do style-level and data-level optimizationss compose?

The research by Tremel et al.~\cite{tremmelMapLibreTileNext2025} has demonstrated that the \ac{MLT} format is structurally superior to the protobuf-based \ac{MVT} format, offering columnar storage, built-in compression, and a richer type system.
...
As the introduction observes, existing tools optimise either data or style, but not both.

\subsection{Problem Statement}\label{sub_mlt_problem}

Two problems remain open after the style-level optimizationss of Experiments~1 and~2:

\begin{enumerate}
    \item \textbf{Tiles carry data the style never uses.}
    ...
    A specific rendering style typically references only a fraction of these.
    Unused columns are transferred, decoded, and discarded on every tile request - wasted bandwidth and CPU.

    \item \textbf{Encoders ignore data characteristics.}
    ...
    Furthermore, safety-critical libraries such as FastPFor~\cite{fastpfor} explicitly disclaim input validation.
\end{enumerate}

\subsection{Approach}\label{sub_mlt_approach}

\cref{sub_data_optimizationss} provides a high-level overview of the data-level techniques; this section details the algorithms and their experimental configuration.
The data-level optimizations pipeline operates in three stages that mirror the structure of a database query optimiser: \emph{logical} rewriting (what data to keep), \emph{physical} strategy selection (how to encode it), and \emph{cost-based} competition (which encoding is smallest).

\begin{enumerate}
    \item \textbf{Style-driven tile shaving} (\cref{sub_tile_shaving}): the optimised style from Experiments~1-2 is analysed to produce a \emph{tile pruning advisory} that specifies, per source-layer and zoom level, which properties, geometry types, and property values to retain.
...
\noindent Stage~1 is the key \emph{style-data co-optimizations}: the style determines what data survives, and the style-level optimizationss from Experiments~1-2 cascade into smaller advisories (fewer live layers means fewer referenced properties) that produce smaller tiles.
Stage~2 uses data statistics to improve compressibility.
Stage~3 is a data-only optimizations that adapts encoding to the statistical properties of the data after pruning and transformation.

\subsubsection{Stage 1: Style-Driven Tile Shaving}\label{sub_tile_shaving}
...
The style optimizationss from Experiments~1-2 thus \emph{cascade} into data-level savings.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Style optimizations
        \node[style] (sopt) {Style optimizations\\{\scriptsize (Experiments 1-2)}};
        \node[data, right=1.0cm of sopt] (stats) {Tile statistics};

        % Advisory
        \node[both, below=0.8cm of sopt, xshift=1.8cm] (adv) {Tile pruning advisory};
        \draw[arrow] (sopt) - node[left, label] {optimised style} (adv);
        \draw[arrow] (stats) - node[right, label] {property/geometry stats} (adv);

        % Tile processing
        \node[data, below left=0.8cm and 0.3cm of adv] (mvt) {\ac{MVT} tile};
        \node[data, below=0.8cm of adv] (prune) {Prune \& transform};
        \node[data, below right=0.8cm and 0.3cm of adv] (mlt) {\ac{MLT} tile};

        \draw[arrow] (adv) - (prune);
        \draw[arrow] (mvt) - (prune);
        \draw[arrow] (prune) - node[right, label] {re-encode} (mlt);

        % Cascade annotation
        \draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick, gray]
            ($(sopt.south west)+(0,-0.15)$) - ($(adv.south east)+(0,-0.15)$)
    ...
    \end{tikzpicture}
    \caption{Style-driven tile shaving pipeline.  The optimised style from Experiments~1-2 and tile statistics jointly produce a pruning advisory.  Each \ac{MVT} tile is pruned according to the advisory, transformed, and re-encoded as \ac{MLT}.  Style-level dead layer elimination cascades into smaller advisories and thus smaller tiles}
    \label{fig:tile_shaving_pipeline}
\end{figure}

\subsubsection{Stage 2: Data Transformations}\label{sub_data_transforms}
...
\subsubsection{Stage 3: Automatic Encoding Strategy Selection}\label{sub_encoding_strategies}

The pruned and transformed data is re-encoded into the \ac{MLT} columnar format.
The Rust-based encoder reformulates the encoding problem as a \emph{multi-strategy competition} inspired by profile-guided optimizations in compilers: rather than committing to a single encoding plan, the encoder tries multiple strategies, measures the output size of each, and selects the smallest result.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/arch_enc}
    \caption{Encoder and decoder data-flow in \texttt{mlt-core}.
    \emph{Decoding} (left) descends from an input \texttt{\&[u8]} slice through \texttt{Raw*} (zero-copy views over the wire format), \texttt{Parsed*} (lazily decoded, still columnar), and finally \texttt{Tile*} (row-oriented, ready to consume).
    \emph{Encoding} (right) ascends from a row-oriented \texttt{Tile*} through \texttt{Staged*} (columnar, not yet encoded) and \texttt{Encoded*} (wire-ready byte buffers) into an \texttt{impl Write} sink via a final zero-copy step.
    Within the encoding path, a \emph{Profile$\leftrightarrow$Optimiser$\leftrightarrow$Encoder} feedback loop implements the multi-strategy competition described in this section: the profile summarises each \texttt{Staged*} stream, the optimiser uses those summaries to trim the candidate search space and generate per-stream encoder guesses, and the encoder fully determines the bytes committed to \texttt{Encoded*}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, draw, rounded corners=2pt, minimum width=2cm, minimum height=0.65cm, font=\small, align=center},
        input/.style={box, fill=gray!15},
        strategy/.style={box, fill=blue!10, minimum width=1.6cm},
        encoder/.style={box, fill=orange!10, minimum width=2.4cm},
        decision/.style={diamond, draw, fill=yellow!15, aspect=2.5, font=\small, inner sep=1pt},
        output/.style={box, fill=green!15},
        arrow/.style={-{Stealth[length=2.5mm]}, thick},
        label/.style={font=\scriptsize\itshape, text=gray},
    ]
        % Input
        \node[input] (layer) {TileLayer01};

        % Strategy candidates
        \node[strategy, below left=1.0cm and 1.2cm of layer] (s1) {Unsorted};
        \node[strategy, left=0.3cm of s1] (s0) {Hilbert};
        \node[strategy, right=0.3cm of s1] (s2) {Morton};
        \node[strategy, right=0.3cm of s2] (s3) {ID};

        % Pruning annotation
        \node[above=0.1cm of s0, font=\scriptsize, text=gray, anchor=south] (prune) {bbox heuristic};
        ...
        % Encoding trials
        \node[encoder, below=0.8cm of s1] (enc) {Encode all columns\\{\scriptsize (per-stream competition)}};
        ...
        \draw[arrow] (enc) - node[right, label] {measure \texttt{total\_len()}} (cmp);

        % Output
        \node[output, below=0.7cm of cmp] (best) {Keep smallest};
        \draw[arrow] (cmp) - node[right, label] {yes: update best} (best);
        \draw[arrow] (cmp.west) - ++(-1.2,0) |- node[near start, above, label] {no: discard} (enc.west);
    \end{tikzpicture}
    \caption{Multi-strategy sort competition.  The layer is encoded under each candidate sort order; the encoding with the smallest \texttt{total\_len()} is retained.  Spatial strategies are pruned by the bounding-box heuristic for large, widely-spread layers.  Within each trial, per-stream encoding competitions select the best integer encoder for every data stream}
...
\subsection{Evaluation}\label{sub_mlt_evaluation}

The central evaluation question for this experiment is whether style-level and data-level optimizationss \emph{compose} - that is, whether optimising the style first, then encoding the resulting tile set, produces better results than either optimizations category in isolation.
This directly addresses Research Goal~R1 (to what extent does data-style co-optimizations reduce tile size and improve rendering).

The evaluation uses a cumulative design with five configurations, each adding one optimizations layer:
...
\begin{enumerate}
    ...
    \item \textbf{Style + shaving + MLT encoding}: the full pipeline with automatic encoding strategy selection (sort competition, per-stream profiling, string interning).
\end{enumerate}

\noindent Configurations~2-4 isolate the interaction effect: if the combined reduction of style+shaving exceeds the sum of style-only and shaving-only, the two optimizations categories are \emph{synergistic}; if less, they are partially redundant.

For each configuration, two classes of metrics are collected:

\begin{itemize}
    \item \textbf{Tile transfer metrics}: encoded tile size (raw, gzip, Brotli) per zoom level, total tileset volume, and encoding time per tile.
    \item \textbf{Rendering metrics} (shared with Experiments~1-2): load time, frames per second, frame time percentiles (p50, p95, p99), and time to first tile - enabling direct comparison of data-level contributions against expression-level and structural contributions in the combined analysis (\cref{sec_exp_combined}).
\end{itemize}
...
% The per-zoom cascade data is visualised in \cref{fig:size_total_per_zoom}.
% The intermediate shaved-MVT mbtiles (configs~3/4) were not persisted, so exact
% per-zoom numbers for those configurations cannot be populated in tabular form.

\subsection{Results}\label{sub_mlt_results}

\cref{fig:size_total_per_zoom} plots total tile size per zoom level across the five configurations; the vertical gap between \enquote{style-only} and \enquote{style+shaving} is the cascade effect of style-level dead elimination and metadata refinement on the tile data, while the gap between \enquote{style+shaving} and \enquote{style+shaving+MLT} is the contribution of the Rust encoder's strategy selection on top of the shaved content.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/size_total_per_zoom}
    \caption{Total tile size per zoom level across configurations~1-5.  The gap between \enquote{style-only} (config~2) and \enquote{style+shaving} (config~4) quantifies the cascade effect; the gap between config~4 and config~5 quantifies the contribution of MLT encoding optimizationss}
    \label{fig:size_total_per_zoom}
\end{figure}
...
\bottomrule
\end{tblr}
\caption{Total Germany OpenMapTiles tileset size (zoom 0-14, \num{256516} tiles) for the three encoders under four wire compressions.  The \ac{MLT}-Java column reports the baseline published in~\cite{tremmelMapLibreTileNext2025}; the \ac{MLT}-Rust column reports the Rust encoder developed in this thesis.  Raw CSV in \texttt{thesis/scripts/data/tile\_sizes.csv}}
...
This isolation directly answers the question posed at the start of \cref{sub_mlt_approach}: the \ac{MLT}-Rust contribution is not in the columnar format alone, but in the joint strategy-selection layer that sits on top of it.
...
\begin{figure}[htbp]
    ...
    \caption{Interaction between style optimizations and tile shaving.  If the combined reduction exceeds the sum of individual reductions, the optimizationss are synergistic - the style cascade produces advisory savings that shaving alone cannot achieve}
...
\paragraph{The cascade effect}
The most significant finding of this experiment is whether style-level and data-level optimizationss are synergistic.
...
This cascade - from style simplification through advisory shrinkage to tile reduction - is the concrete realisation of the \enquote{data-style co-optimizations} promised in the thesis title.
...
This observation connects data-level optimizations to the multi-objective framework of \cref{ch_theoretical_background}: there is no single optimal tile - the optimal encoding depends on the rendering style, which is a design choice.
...
For deployments where a single style dominates traffic, style-driven shaving is strictly beneficial; for multi-tenant tile servers hosting dozens of styles, shaving should be applied per style and the resulting tile variants cached independently (the approach of Mapbox's closed-source \enquote{style-optimized vector tiles} service~\cite{mapboxStyleOptimizedTiles}).
...
The thesis therefore quantifies the size reductions achievable \emph{without} touching geometry at all, which understates the total optimizations headroom a production pipeline could reach by composing this work with an upstream simplifier.
...
The preceding experiments evaluate optimizations effectiveness on a curated corpus of 15 styles.
...
Simple styles with fewer than 50 layers offer limited optimizations headroom - they are already compact - while verbose institutional styles with several hundred layers and deeply nested expressions offer the most room for improvement.
...
Because the generalisation corpus does not include tile data, stats-driven passes (dead elimination with geometry-type matching, stats-driven constant folding, selectivity reordering) and data-level optimizationss (tile shaving, re-encoding) cannot be applied.
The reported reductions therefore represent a \emph{lower bound} on the full optimizations potential.
...
No style in the corpus shows a size \emph{increase} after optimizations, confirming that the passes are safe in the general case.


\section{Combined Results}\label{sec_exp_combined}

This section presents the full pipeline ablation - all 17 cumulative steps from baseline through tile rewriting - and analyses the interactions between optimizations categories.
The combined view is essential because the three optimizations categories (expression, structural, data) interact in ways that are invisible when each is evaluated in isolation: style-level dead elimination cascades into smaller tile pruning advisories, and metadata refinement enables more aggressive zoom-stop pruning in the data pipeline.
...
\caption{Per-style summary: baseline and optimised style size (gzip), percentage reduction, and change in load time and FPS (median across scenarios). Americana excluded (incomplete data). Styles below the rule include tile-level optimizationss (steps~16-19)}
...
Research Goal~R3 asks how expensive the optimizationss are in terms of preprocessing time.
The optimizations pipeline is an \emph{offline} stage: a style is optimised once and served many times, and tiles are re-encoded once per tileset build.
...
\emph{Data profiling} (\texttt{encoder/\allowbreak stream/\allowbreak optimizer.rs}) samples a contiguous mid-stream window to rule out logical encodings that cannot possibly win before any full encode pass is run, reducing the per-stream candidate set from the na\"ive worst case of eight to an average of two to three fully executed trials.
\emph{\ac{FSST} warm-start caching} (\texttt{fsst\_cache} in the string encoder, \texttt{mlt\allowbreak-core/\allowbreak src/\allowbreak encoder/\allowbreak property/\allowbreak strings.rs}) trains a symbol table exactly once per distinct unique-value set and reuses it across every sort-strategy trial, so the trained compressor is not rebuilt when the same column is re-encoded under a different feature ordering; the same applies to Morton dictionary metadata and HyperLogLog vertex-uniqueness sketches.
...
The full pipeline ablation reveals three qualitatively different mechanisms by which optimizationss improve rendering performance.
...
The cumulative and marginal views in \crefrange{sub_combined_ablation}{sub_combined_marginal} report each optimizations pass on one axis at a time.
This section treats the already-measured set of pass configurations as points in a two-axis objective space and reports the non-dominated subset in the sense of \cref{sec_multi_objective_optimizations}.
The analysis is deliberately \emph{post-hoc}: it reads the existing benchmark output and recomputes nothing in the encoder or the style optimiser.
No new configurations are measured; no internal knob is retuned; the cumulative pass ordering defended in \cref{sub_structural_interactions} remains the primary recommendation, and Pareto dominance here is used to illuminate the trade-off space rather than to pick a different order.

The section is narrower than a naive reading of Research Goal~R3 would suggest.
An honest noise-floor analysis of the benchmark corpus (\cref{sub_pareto_noise}) shows that only \texttt{gzip\_bytes} among the four recorded candidate axes carries a signal strong enough to support per-step frontier claims at the current sample size.
The remaining three axes - \texttt{loadMs}, \texttt{meanFrameMs}, and \texttt{peakHeapMB} - are excluded from the frontier itself and the gap is recorded as a concrete future-work item in \cref{sec_conclusion_future_work}; a wall-clock preprocessing-cost axis and a CPU-time energy proxy were also considered and dropped, for the reasons given in their respective paragraphs below.
...
A candidate axis can only support a per-step Pareto claim if its cross-variant signal is visibly larger than its within-cell noise; otherwise the non-dominated points are selected by the noise rather than by the optimizationss.
...
\csvreader[
  ...
  table head={\toprule
    ...
    \midrule},
  table foot={\midrule
    \multicolumn{6}{l}{\footnotesize Generated by
    \texttt{thesis/scripts/pareto\_analysis.py}.} \\
    \bottomrule},
  head to column names,
...
\subsection{Objective Space and Decision Space}\label{sub_pareto_scope}

The frontier therefore uses two axes, both deterministic on the current data:

\begin{itemize}
    \item \textbf{\texttt{gzip\_bytes}}~(bytes)  -  transfer size of the style document after gzip compression at level~9.
    Scenario-invariant; already recorded by the benchmark harness for every (style, variant) pair (cf.\ \cref{sub_metrics}).
    \item \textbf{\texttt{pass\_count}}~(unitless)  -  the number of optimiser passes enabled for a variant.
    This is a first-order proxy for preprocessing cost (more passes means more work in the optimiser, modulo inter-pass sharing) and simultaneously a proxy for pipeline complexity (every pass enabled is a potential source of regressions, a contribution to build-time fragility, and a maintenance burden).
    Reading the axis as \enquote{enabled passes} rather than as \enquote{milliseconds of preprocessing time} is deliberate: the wall-clock preprocessing cost is also collected at per-variant granularity by the \texttt{preprocessing\_ms} timing hook added to the benchmark harness (see the note at the end of \cref{sub_ablation}), but the existing \texttt{jsonl} corpus predates that hook, so the present analysis falls back on the deterministic integer count.
    When re-run on a corpus that includes \texttt{preprocessing\_ms}, the analysis script replaces the integer axis with the measured milliseconds and the frontier tightens accordingly.
\end{itemize}

The configurations compared are exactly those the harness already measures.
The \emph{cumulative trajectory} contributes up to 18~points per style - a baseline with no passes enabled followed by up to 17 points where step~$k$ enables passes~$1 \ldots k$ in the fixed order listed in \crefrange{tab:expression_passes}{tab:metadata_passes}.
The \emph{isolated mode} contributes a further point for each single-pass configuration measured on top of the baseline.
This decision space is not the full $2^{17}$ mix-and-match space of pass subsets and not the $17!$ space of pass permutations; both are unmeasured and out of scope for this post-hoc analysis, and both are flagged in \cref{sec_conclusion_future_work}.
Internal encoder knobs - the MinHash similarity threshold at \texttt{rust/\allowbreak mlt\allowbreak-core/\allowbreak src/\allowbreak encoder/\allowbreak property/\allowbreak shared\_dict.rs:29}, the \ac{FSST} gating decision, and the tessellation boolean - are held fixed at their defaults throughout.
...
\begin{figure}[htbp]
    ...
    Generated by \texttt{thesis/scripts/pareto\_analysis.py} from the \texttt{jsonl} output of the benchmark harness using the latest complete ablation session per style}
...
\paragraph{What this section answers}
Research Goal~R1's transfer-size axis is answered at per-style granularity by \cref{fig:pareto_per_style}: across the benchmark corpus, baseline-to-final cumulative gzip reductions range from roughly \qty{3}{\percent} on already-compact styles (\texttt{bright}, \texttt{liberty}) to \qty{24}{\percent} on verbose styles (\texttt{klokan-basic}, \texttt{osm-bright}, \texttt{toner}), and the isolated-mode frontier exposes which single pass captures the largest share of that reduction per style.
Research Goal~R3's \emph{preprocessing cost} axis is answered as a pass-count proxy now, and will be answered as a wall-clock measurement once the \texttt{preprocessing\_ms} timing hook in \texttt{tests/bench/run.ts} populates a new benchmark run; the analysis script auto-detects the column and switches axes without requiring further edits to this section.

\paragraph{What this section does \emph{not} answer}
Research Goal~R1's \emph{rendering-performance} axis, the \emph{decoding latency} and \emph{rendering throughput} axes of R3, the \emph{energy} axis of R3, and the memory axis are \emph{not} answered by the frontier in this section, because the within-cell noise on the corresponding metrics is of the same order as the cross-variant signal (\cref{tab:pareto_noise}).
Answering them on this framework requires either a run-count increase of roughly $5$-$10\times$ per cell, or a dedicated benchmark host with reduced OS and GPU thermal variance, or renderer-internal instrumentation that isolates decoder time from the surrounding fetch-and-paint work - and in practice probably some combination of all three.
These constraints are acknowledged here rather than hidden, and each maps onto a concrete future-work item in \cref{sec_conclusion_future_work}.

\paragraph{Consistency with earlier sections}
The restriction to a two-axis size-cost frontier is deliberately \emph{narrower} than a literal reading of Research Goal~R3.
The qualitative rendering-performance and energy claims elsewhere in the thesis - \cref{sub_combined_ablation}'s cumulative ablation, \cref{sub_combined_discussion}'s mobile-battery discussion, \cref{sub_preprocessing_cost}'s top-down cost model - remain the authoritative answers on those axes; they are not contradicted by the present analysis because they operate on end-to-end (baseline versus full pipeline) aggregates, where the larger effect size makes them robust to the same noise that defeats a per-step frontier.

\paragraph{Methodology limits}
First, only the cumulative trajectory and isolated mode are measured: the full $2^{17}$ subset space and the $17!$ permutation space are unmeasured, and nothing in this section speaks to whether a reordered sequence would produce a different frontier.
Second, the per-stream encoder competition inside \texttt{mlt-core} remains single-objective (minimum \texttt{total\_len()}) by construction, so the outer Pareto layer cannot recommend a \emph{different} per-stream encoding even when a deployment would prefer to trade a few bytes for decoder simplicity; the encoder criterion stays size-only, as noted in \cref{sub_data_optimizationss}.
Third, the analysis operates on the latest complete ablation session per style: earlier benchmarks used a slightly different \texttt{ABLATION\_STEPS} ordering in \texttt{tests/bench/run.ts}, so mixing sessions would merge semantically identical passes under different numeric step indices; the filter is documented in \texttt{pareto\_analysis.py} and is logged in the script's \texttt{pareto\_summary.json} output so that a reviewer can verify which session was used for which style.


\section{Visual Correctness Validation}\label{sec_visual_correctness}

A fundamental requirement of the optimiser is that all transformations are visually lossless: the optimised style and tile data must produce the same rendered output as the unoptimised baseline within the quantification of the visual-equivalence constraint in \cref{sec_problem_statement}.
...
Across all comparisons, pixelmatch reported zero differing pixels at a strict (zero-tolerance) threshold, confirming that every optimizations pass preserves visual identity on the software rasteriser.
...
The underlying optimizations invariants (overzoom-aware metadata refinement, painter's-order-pre\-serving layer merging) are, however, zoom-invariant by construction.
Second, the equivalence holds for the software rasteriser.
The benchmark harness runs the GPU backend (ANGLE/Vulkan) in parallel for performance measurement, and no GPU-only divergence appears in the benchmark traces, but the claim \enquote{pixel-exact on the GPU} is neither asserted nor required - style and tile transformations cannot introduce GPU-only divergences not already present in the software rasteriser, because both backends consume the same post-optimizations inputs.
...
The 15-run median provides robustness against outliers but does not quantify sampling uncertainty directly; bootstrap \qty{95}{\percent} confidence intervals over the 15-run samples would be the natural extension, and are tracked as future work.
