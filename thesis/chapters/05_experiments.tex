...
\cref{sec_exp_mlt} assesses data-level optimisations, measuring how style-level dead layer elimination narrows the tile pruning advisory, how tile shaving reduces tile size, and how the encoding strategy competition delivers the shaved data to the wire.
...
This experiment evaluates the eight expression-level passes of \cref{sub_expression_optimisations} - the rewrites that act locally on each style's JSON expressions and run to a fixpoint.
...
The full rendering and static-size metric set is described in \cref{sub_metrics}.
...
The independence assumption underlying the selectivity estimates is acknowledged as inaccurate for correlated map predicates (\cref{sec_query_optimisations}), and the empirical benefit may be negligible for styles whose filters are already short.
...
Beyond their direct rendering benefit, expression-level simplifications feed into the data-level pipeline evaluated in \cref{sec_exp_mlt}.
...
The magnitude of this interaction is quantified in \cref{sec_exp_mlt}.


\section{Experiment 2: Structural Optimisations}\label{sec_exp_structural}

Where the previous experiment rewrote local expressions, this one evaluates the passes that reason across layers and sources - the structural family of \cref{sub_structural_optimisations}.

\subsection{Pass Interactions}\label{sub_structural_interactions}

Several pairwise dependencies between structural passes shape what the cumulative ablation in \cref{sub_structural_results} measures.
...
The full ordering rationale is given in \cref{sub_pipeline}; the practical consequence here is that the marginal contribution of each later pass in the ablation will reflect work that an earlier pass enabled, not just the pass itself.

\subsection{Evaluation}\label{sub_structural_evaluation}

Structural passes correspond to ablation steps~8-15.
In addition to the rendering metrics from Experiment~1 (\cref{sub_metrics}), the structural evaluation reports complexity metrics: layer count, filter count, and total expression node count.
...
Symbol layers are excluded from merging because MapLibre's collision detection operates per-layer (the soundness argument is given in \cref{sub_structural_optimisations}).
...
This interaction is characterised fully in \cref{sub_mlt_discussion} and quantified in \cref{sub_mlt_results}; the aggregate data shows that the composition is additive rather than synergistic, but the advisory-narrowing effect is a necessary precondition for style-dependent tile shaving.
...
The data-level pipeline applied here (\cref{sub_data_optimisations}) takes the optimised style from Experiments~1-2 and uses it to drive the tile pruning advisory and the encoding strategy competition that follows.
...
\begin{enumerate}
    ...
    The unused remainder is style-dependent, so the savings achievable by tile shaving (\cref{sub_tile_shaving}) co-vary with the upstream style optimisations:
    ...
    After shaving, the surviving data has fewer distinct values, longer runs, and smaller string vocabularies, which a data-adaptive encoder (\cref{sub_encoding_strategies}) can exploit but a fixed-plan encoder cannot.
\end{enumerate}
...
This section quantifies the interaction between style-level and data-level optimisations previewed in \cref{sub_structural_discussion}: style-level optimisations narrow the pruning advisory, and the following paragraphs measure the extent of this effect before turning to the encoding evaluation.
...
The decoder is engineered to amortise its per-column dispatch over per-feature work (\cref{sub_encoding_strategies}); empirically, benchmarks on the full Germany tileset show end-to-end rendering metrics comparable to or better than \ac{MVT} at matched workloads, and no negative effect appears in the sustained frame-time metrics of \cref{sub_combined_marginal}.
...
However, the aggregate data shows no super-additive interaction at the median level (\cref{sub_mlt_results}): the combined style+shaving reduction is approximately the sum of the individual reductions, not greater.
...
No single sort order dominates: across the Germany tileset, the per-layer winners are split between Hilbert, Morton, and ID, confirming that the nested competition (\cref{sub_encoding_strategies}) materially affects which encoding survives the inner per-stream contest rather than being a redundant outer loop.
...
The four complexity metrics tracked across ablation steps (\cref{sub_metrics}) give a structural explanation for the observed rendering improvements:
...
Where the same step appears across two complexity axes, the connection to the corresponding rendering metric is in \cref{sub_combined_marginal}.
...
The three channels target non-overlapping costs - simplification cannot shrink tiles and shaving cannot prune expressions - so they compose additively in the aggregate, as already established quantitatively in \cref{sub_mlt_results}.
