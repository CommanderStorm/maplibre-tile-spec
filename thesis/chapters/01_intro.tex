...
\cref{ch:methods} states the joint problem formally in \cref{sec:problem_statement} - producing $(S', T')$ from $(S, T)$ under the pixel-equivalence constraint of \cref{eq:visual_equivalence} - and details the three-level pipeline that solves it: expression rewrites over raw JSON, structural passes over a typed style model, and data-level passes that emit a style-driven pruning advisory and re-encode in \ac{MLT} with automatic strategy selection.
