...
Before developing an optimiser that bridges this gap, we survey the state of the art along four dimensions that correspond to the levels at which the optimiser will operate.
...
For each area, we identify what existing tools achieve and their shortcomings, building toward the composite gap that motivates the co-optimisation approach we develop in this thesis.
...
The problem structure is directly analogous to the layer merging pass we develop in this thesis: both address merging declarative style rules that differ in selectors (CSS) or filters (map styles) but share properties, subject to ordering constraints imposed by the cascade (CSS) or painter's algorithm (map rendering).
...
The recent formalisation of vector tile delivery in the OGC API-Tiles standard~\cite{OGCAPITiles} underscores the need for optimisation techniques that are compatible to standardised, open infrastructure rather than tied to a single vendors' API.

Notably, no existing open-source tool or published resarch performs \emph{systematic composition} of style and data optimisations -- the unified $(S, T) \to (S', T')$ optimisation we formalise in \cref{sec:problem_statement}.
This means that using tile statistics to inform style simplification, and using the simplified style to derive a pruning advisory for the data is novel territory.
We fill this gap by applying compiler and database optimisation techniques to the map style and tile data as a unified optimisation target, measuring the interaction between levels through a cumulative ablation methodology that isolates each pass's contribution and quantifies whether the composition is additive or synergistic.
We quantify the magnitude of this gap in \cref{ch:experiments}, demonstrating that composing these techniques reduces load times by up to \qty{75}{\percent} and tile volume by \qty{28}{\percent}.

In the following chapter we introduce the theoretical foundations that these techniques draw on:
