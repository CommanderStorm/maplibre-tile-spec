...
MapLibre GL~JS\footnote{\url{https://maplibre.org/maplibre-gl-js/docs/}, last accessed 2026-06-28} and Mapbox GL~JS\footnote{\url{https://www.mapbox.com/mapbox-gljs}, last accessed 2026-06-28} deliver these maps by streaming tiled geometry to the browser and letting a client-side style document drive a \ac{WebGL}- or \ac{WebGPU}-based rendering pipeline.
...
\begin{enumerate}
    ...
    Expression passes work on raw style \ac{JSON}, structural passes on a typed style model, and data passes on tile payloads.
    Each representation is chosen to match its level's reasoning requirements (\cref{sub:pipeline}).
