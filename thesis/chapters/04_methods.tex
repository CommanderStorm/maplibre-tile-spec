...
In \cref{sec:problem_statement} we formalise the optimisation problem and the visual-equivalence constraint that all passes must satisfy.
\cref{sec:scope} scopes the work to MapLibre GL~JS and describes the implementation platform.
\cref{sec:approach} then describes the three-level pipeline, and the subsequent sections detail the individual passes at each level.
...
\noindent The following paragraphs describe the algorithm and rationale for each structural pass.
...
\noindent Additionally, we compute the following static metrics for each optimised style without running the renderer:
