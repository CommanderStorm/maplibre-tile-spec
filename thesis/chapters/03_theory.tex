...
A term to which no rule applies is descriptively a \emph{fixpoint} of the rewrite function and equivalently a \emph{normal form} of the underlying rewrite system.
...
The right theoretical lens is therefore the \ac{TRS} pair of \emph{termination} (whether every reduction sequence reaches a normal form) and \emph{confluence} (whether all sequences reach the same one), rather than the lattice-theoretic monotone-convergence framing of classical dataflow fixpoint iteration~\cite{ahoCompilersPrinciplesTechniques2007}.
Our rewrite function is not monotone in any standard ordering.
\Cref{sub:expression_optimizations} gives a lexicographic measure $\mu$ on which most rules strictly decrease, together with the two rules (De~Morgan and distributive factoring) that can temporarily increase it, so a Knaster-Tarski-style convergence theorem does not apply here.
In practice the loop reaches a normal form within a small number of iterations, and we impose a hard cap of eight iterations as a safety bound rather than as a convergence guarantee.
