...
\begin{enumerate}
    \item \textbf{Expression passes} (\cref{sub:expression_optimisations}) operate directly on the JSON representation of style expressions, applying peephole rewrites, constant folding, and algebraic simplifications without deserialising the style into typed structures.
...
\begin{lstlisting}[
	language=TypeScript,
	escapeinside={(*@}{@*)},
	caption={MapLibre GL-JS  symbol placement (\texttt{src/style/pauseable\_placement.ts:90-127})},
	label=lst:placement]
...
\begin{figure}[H]
    ...
    \resizebox{\linewidth}{!}{\hfuzz=50pt\begin{tikzpicture}[
        ...
        % Input layers (left)
        \node[layer, fill=roleSrc!20]   (l0) {\stepcirc{1}~line:\\ \texttt{filter: ["get", "class"] == "motorway"}\\\texttt{line-color: \#e892a2}\\\texttt{line-width: 3}};
        \node[layer, fill=roleData!25, below=0.5cm of l0] (l1) {\stepcirc{2}~line:\\\texttt{filter: ["get", "class"] == "primary"}\\\texttt{line-color: \#fcd6a4}\\\texttt{line-width: 2}};
        \node[layer, fill=roleMir!30, below=0.5cm of l1] (l2) {\stepcirc{3}~line:\\\texttt{filter: ["get", "class"] == "secondary"}\\\texttt{line-color: \#f7fabf}\\\texttt{line-width: 1.5}};
