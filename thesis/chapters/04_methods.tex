...
For example, the stats-driven folding pass cannot soundly remove a \texttt{match} arm without a full census of the tile data, because the arm may be reached by features the statistics have not observed;
the pass is consequently gated on a sample rate of~1.0.
Similarly, the metadata-refinement pass must reason about overzoom (\cref{sec:tiles}) when tightening \texttt{maxzoom}, because an overzoomed tile counts as a viewing state in~(\ref{eq:visual_equivalence}).
...
Expression kind normalization & none     & Rewrite negated comparisons to their direct form, e.g.\ \texttt{["!",["==",a,b]]} $\to$ \texttt{["!=",a,b]} [operator strength reduction] \\
...
\begin{itemize}
    ...
    \item \textbf{Data-ramp pruning}: for \texttt{step} and \texttt{interpolate} expressions driven by a numeric property with known data range $[\mathrm{min},\, \mathrm{max}]$, the pass prunes stops outside the observed range.
    In a \texttt{step} expression, it removes stops with thresholds above $\mathrm{max}$; when the property is present in all features, it absorbs stops below $\mathrm{min}$ into the default.
    In an \texttt{interpolate} expression, it prunes stops outside the $[\mathrm{min},\, \mathrm{max}]$ interval symmetrically.
\end{itemize}
...
Within \texttt{match} expressions, the pass merges duplicate arms and removes arms whose output equals the fallback.

\begin{sloppypar}
The pass detects boolean \texttt{match} patterns (\texttt{["match",\allowbreak~x,\allowbreak~labels,\allowbreak~true,\allowbreak~false]}) and rewrites them to \texttt{["in",\allowbreak~x,\allowbreak~["literal",\allowbreak~labels]]}.
...
For variadic boolean expressions (\texttt{all}, \texttt{any}), the pass reorders predicates by estimated selectivity to exploit short-circuit evaluation, applying the selectivity-estimation framework of \cref{sec:query_optimizations}.
For \texttt{all} (conjunction), it sorts predicates by ascending selectivity (the predicate most likely to be false is evaluated first).
For \texttt{any} (disjunction), it sorts predicates by descending selectivity (the predicate most likely to be true is evaluated first).
...
\begin{equation}\label{eq:selectivity_compound}
    \begin{aligned}
        \mathrm{sel}(\texttt{all} \quad p_0 \ldots p_i) &= \prod_i \mathrm{sel}(p_i) \\[4pt]
        \mathrm{sel}(\texttt{any} \quad p_0 \ldots p_i) &= 1 - \prod_i \bigl(1 - \mathrm{sel}(p_i)\bigr) \\[4pt]
    \end{aligned}
\end{equation}
...
In the first, the pass factors two equality tests within an \texttt{any} node and rewrites them to a compact \texttt{in} expression.
...
["all",
  ...
  ["in",
    ...
    ["literal",["road","rail"]]
		...
		\caption{Expression rewriting via distributive factoring and \texttt{any}$\to$\texttt{in} promotion.}
		\label{fig:expr_a}
...
\emph{Filter-based extraction} walks the filter expression tree and recognizes zoom predicates of the form \texttt{[">=",~["zoom"],~$z$]} or \texttt{["<=",~["zoom"],~$z$]}.
Within an \texttt{all} node, it intersects the extracted bounds (the tighter of the bounds applies); within an \texttt{any} node, it unions them (the layer is visible if any branch is satisfied).
...
\begin{equation}\label{eq:zoom_bounds}
    ...
    \end{aligned}
\end{equation}
Within an \texttt{all} node, it intersects extracted intervals:
\[
z^{*} = [l_1, u_1] \land [l_2, u_2] = [\max(l_1, l_2),\; \min(u_1, u_2)]
\]
Within an \texttt{any} node, it unions them:
\[
z^{*} = [l_1, u_1] \lor [l_2, u_2] = [\min(l_1, l_2),\; \max(u_1, u_2)]
\]
After extraction, the pass removes the consumed zoom predicates from the filter, often simplifying it to \texttt{true}.
...
\emph{Stats-driven refinement} tightens bounds using actual tile coverage: if a source layer's first tile appears at zoom~4, the pass raises the layer's \texttt{minzoom} to~4 regardless of what the filter permits, since there is no data to render below that zoom.
This refinement only tightens bounds; it never loosens them due to \cref{eq:visual_equivalence}.

A subtle but load-bearing correctness concern is MapLibre's \emph{overzoom} behavior: when a client requests a tile at a zoom level beyond a source's \texttt{maxzoom}, the renderer reuses the nearest available tile from a lower zoom and upscales it.
...
(1)~it removes empty \texttt{paint} and \texttt{layout} sections (which may have become empty after default stripping or stats-driven folding);
(2)~it removes invisible layers (\texttt{visibility:~none}, or zero opacity that is not an expression);
(3)~it removes trivially true filters (\texttt{["literal",~true]}), which are semantically equivalent to no filter;
...
\Cref{fig:layer_merge_example} works through exactly this case: three adjacent \texttt{line} layers keyed on \texttt{class} collapse into a single layer whose filter, \texttt{line-color}, \texttt{line-width}, and synthesised \texttt{line-sort-key} are all \texttt{match} expressions over the same discriminator, with Phase~1 (mergeability) and Phase~3 (identical zoom ranges) discharged trivially.
...
After all layer-level passes, this pass tightens source \texttt{minzoom}/\texttt{maxzoom} bounds to the effective zoom range of the layers that reference each source.
...
\begin{figure}[H]
    ...
    \resizebox{\linewidth}{!}{\hfuzz=50pt\begin{tikzpicture}[
        ...
        % Input layers (left)
        \node[layer, fill=roleSrc!20]   (l0) {line\\ \texttt{filter: ["get", "class"] == "motorway"}\\\texttt{line-color: \#e892a2}\\\texttt{line-width: 3}};
        \node[layer, fill=roleData!25, below=0.5cm of l0] (l1) {line\\\texttt{filter: ["get", "class"] == "primary"}\\\texttt{line-color: \#fcd6a4}\\\texttt{line-width: 2}};
        \node[layer, fill=roleMir!30, below=0.5cm of l1] (l2) {line\\\texttt{filter: ["get", "class"] == "secondary"}\\\texttt{line-color: \#f7fabf}\\\texttt{line-width: 1.5}};
    ...
    \end{tikzpicture}}
    \caption[Layer merging example]{Layer merging example.  Three adjacent line layers differing only in filter literal, color, and width are merged via the \texttt{match} pattern (Phase~2).
...
\begin{itemize}
    ...
    For example, if no style layer filters for \texttt{class = "ferry"}, the pass can remove features with that value.
...
For example, the optimizer rewrites a filter \texttt{["==",~["get",~"class"],~"primary"]} to \texttt{["==",~["get",~"class"],~0]} if \texttt{"primary"} is the most frequent value.
