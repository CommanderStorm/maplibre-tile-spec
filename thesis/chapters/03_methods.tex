...
All transformations satisfy the visual-equivalence constraint formalised in \cref{sec_problem_statement}: the optimised output is pixel-identical to the original at every zoom level.
...
Specifically, for layers with more than \texttt{SORT\_TRIAL\_THRESHOLD = 512} features, the spatial trials are pruned when the first-vertex bounding box covers more than \texttt{SPATIAL\_HELP\_COVERAGE = 80\%} of the tile extent on \emph{both} axes.
...
Instead, a \emph{data profiler} samples a contiguous block taken from the middle of the stream: streams with at most \texttt{MIN\_SAMPLE = 512} values are profiled in full, and longer streams are profiled over $\lfloor |s| / 100 \rfloor$ values clamped to $[\texttt{MIN\_SAMPLE},\, \texttt{MAX\_SAMPLE}]$ with \texttt{MAX\_SAMPLE = \num{16384}}.
...
The alternatives mechanism itself is implemented as an RAII stack of \texttt{AltSession} guards: each candidate writes into a sub-range of the encoder's buffers, and when the session's \texttt{Drop} impl fires, it retains only the bytes of the smallest candidate and rolls the buffer cursor back for the others.
