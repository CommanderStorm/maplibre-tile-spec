...
\begin{figure}[htbp]
	...
	\caption{Style size (raw, gzip, Brotli) across ablation steps shows that the effect on this metric exists, but is iterative rather than transformative}
...
A per-style break-even model frames the decision.
...
Two regimes are practically meaningful.
...
A \emph{long-tail style} ($\lambda_i \ll \Lambda_\mathcal{M}$) sees a gap that approaches the full pooled hit rate $h(\Lambda_\mathcal{M}/N)$, because its private cache cannot self-warm on its own thin traffic and reverts to near-cold-start behaviour.
With the benchmark values ($r \approx \qty{3}{\milli\second}$ per tile, $C_f \approx \qty{13.5}{\milli\second}$ under the Rusan et al.~\cite{rusanAssessmentPacketLatency2015} $\frac{\text{tile size}}{\qty{30}{\mega\bit\per\second}} + \frac{\qty{25}{\milli\second}}{2}$ server-side-throttle for a \qty{50}{\kilo\byte} average tile) and a representative pooled hit rate $h(\Lambda_\mathcal{M}/N) \approx 0.9$ for popular tile content, the maximum tolerable hit-rate gap is $r/C_f \approx 6.6\%$.
...
The corollary is the hybrid pattern:
Shave the one or two canonical styles that dominate the pool, and leave the long tail on raw \ac{MVT}.
This recommendation is driven by traffic concentration, not style count - a server hosting fifty styles with one \qty{70}{\percent}-share dominant style behaves the same as one hosting five with the same concentration.
The model omits spatial locality and the second-order effect that peeling the dominant style off the pool cools the cache further for the remaining \ac{MVT}-served styles, both of which would only sharpen the recommendation.
...
Consequently of this large improvement in frame time, previously hidden frame time consistency issues become more obvious as shown in \cref{fig:heatmapjankcount}.
These consistency issues can be mostly attributed to \ac{GC}-pauses.
...
\begin{figure}[tbph]
	\centering
	\includegraphics[width=\linewidth]{figures/heatmap_jankCount}
	\caption{
		Heatmap of how jank frames, i.e. frames times that are much slower than the sorrounding frame times, happen.
		Due to the increased FPS, smaller variations like \ac{GC}-pauses make this metric go up as FPS is higher.
		This does not nessarily have an effect to the user, since real devices render below the 2000FPS marker that we run at, but nevertheless something to keep in mind.}
...
\caption{
	Per-style summary for tile and non-tile optimised styles.
	Styles below the rule include tile-level optimisations (steps~1-19) while above only include  (steps~1-15)
	Baseline and optimised style size (gzip), percentage reduction, and change in load time and FPS (median across scenarios).}
...
After to tile shaving, Expression-level constant folding and structural dead elimination tend to produce the largest marginal improvements in frame time and FPS, as they reduce the number of expressions the renderer must evaluate per feature per frame and the number of layers it must process, respectively.
...
This section measures each of the four additive cost terms $C_\text{style}$, $C_\text{stats}$, $C_\text{adv}$, and $C_\text{enc}$ from the preprocessing cost model (\cref{eq:cost_model}, \cref{sub:preprocessing_cost_model}) on a real tileset, and confirms that the cost-control mechanisms formalised there keep the multi-strategy overhead well below the na\"ive worst case.
...
The measured overhead ratio of the full competition is therefore $342 / 175 = 1.95\times$ in CPU time - substantially lower than the analytical 3-5$\times$ estimate, because the cost-control mechanisms formalised in \cref{sub:preprocessing_cost_model} eliminate most redundant work.
...
This empirically confirms that the trial-and-retain loop costs $k$ encode passes plus $\mathcal{O}(1)$ bookkeeping rather than $k$ encode passes plus $k$ full buffer clones.
...
The effect of this on client side performance is not as big as it could currently, internally, data is transformed to \acsp{MVT} instead of an more efficient in-memory-structure.
...
\emph{Data reduction} (tile shaving, re-encoding) reduces transfer cost: smaller tiles mean shorter download times and lower decode overhead, primarily improving load time and time to first tile, but because of lower \ac{GC}-spend also average frame times.
...
This variance is partially self-inflicted as the use of real GPU hardware (ANGLE/Vulkan) rather than a software rasteriser captures realistic rendering costs, including GPU buffer management and shader compilation, at the expense of slightly higher measurement noise compared to deterministic software rendering.
