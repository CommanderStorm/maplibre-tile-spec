...
\cref{sec_multi_objective_optimisations} discusses multi-objective optimisation, \cref{sec_benchmarking} covers benchmarking methodology, and \cref{sec_problem_statement} formalises the optimisation problem that \cref{ch_methods} addresses.
...
\section{Multi-Objective Optimisation}\label{sec_multi_objective_optimisations}

Map rendering optimisation is inherently multi-objective: reducing tile size, minimising load time, maximising rendering throughput, and controlling memory consumption are competing goals.
Such problems can be formulated as \acsp{MOOP}, where the set of \emph{Pareto-optimal} solutions - configurations for which no objective can be improved without worsening another - forms the \emph{Pareto front}~\cite{paretoCoursDEconomiePolitique1896}.
A range of algorithms exist for approximating the Pareto front~\cite{debFastElitistMultiobjective2002,zitzlerIndicatorBasedSelectionMultiobjective2004,qingfuzhangMOEAMultiobjectiveEvolutionary2007}.
This thesis does not embed a \ac{MOOP} search algorithm; instead, it uses Pareto dominance as a post-hoc analysis tool over the cumulative-ablation result space (\cref{sec_exp_pareto}), restricted to the two axes whose signal exceeds the benchmark noise floor.
