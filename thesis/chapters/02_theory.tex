...
The gap identified in \cref{ch_related_work} - no existing tool composes style rewrites with data-level pruning into a joint pipeline - calls for foundations from several fields: the map rendering architecture that defines what is being optimised, the compiler techniques whose analogues drive the expression-level passes, the query optimisation concepts that inform filter rewriting and predicate reordering, and the columnar encoding strategies that underpin the data-level passes.
...
Although all of these resources can be optimised, tiles are usually the most critical due to their continuous streaming nature as a user moves the map thus higher data volume.
