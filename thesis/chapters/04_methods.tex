...
\texttt{new LayerPlacement(layer)} constructs a fresh instance when a symbol layer's placement begins, and \texttt{delete this.\_inProgressLayer} tears it down before the next layer starts; the underlying \texttt{Placement} object (and its \texttt{CollisionIndex}) is shared across all layers via \texttt{this.placement}, which is what makes the \texttt{LayerPlacement} scope per-layer rather than global.
