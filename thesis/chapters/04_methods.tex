...
The pass strips only plain scalar values; it never removes expression-valued properties, even if they would evaluate to the default at all zoom levels, to avoid the risk of changing renderer behavior when the expression's mere presence affects layout (e.g.\ \texttt{text-radial-offset} suppresses \texttt{text-offset} regardless of its value).
...
Cleanup                      & none      & Remove empty \mlprop{paint}/\mlprop{layout} sections, invisible layers (\texttt{visibility:~none}, zero opacity), trivially true filters, and root-level properties at specification defaults [dead code elimination] \\
...
A \texttt{line} layer can draw both on polygons and lines, but not points.
A \texttt{symbol}/\texttt{circle} layer can draw on polygons, lines and points and is never dead.
...
(2)~it removes invisible layers (\texttt{visibility:~none}, or zero opacity that is not an expression);
...
(3)~any MapLibre runtime behavior that is quantified per layer - collision detection, \texttt{feature-state}, \texttt{*-translate} and similar camera-only properties - must not change its per-layer scope.
...
We accept \texttt{fill-extrusion} layers as merge candidates in principle, but the current implementation inherits the same sort-key restriction; in practice their relative ordering is dominated by the depth buffer rather than the painter's algorithm, so the renderer uses the \texttt{fill-extrusion-sort-key} property to break ties at co-planar surfaces only.
...
\emph{Phase~2} (match-pattern detection) checks whether all filters within a candidate group match the pattern \texttt{[\mlop{"=="},~[\mlop{"get"},~P],~literal]} on the same property~$P$.
...
The pass synthesises sort keys only for layer types that support the \texttt{*-sort-key} property and that do not already define one.
...
\begin{figure}[H]
    ...
    \resizebox{\linewidth}{!}{\hfuzz=50pt\begin{tikzpicture}[
        ...
        % Input layers (left)
        \node[layer, fill=roleSrc!20]   (l0) {line\\ \texttt{filter: [\mlop{"get"}, \mlval{"class"}] == \mlval{"motorway"}}\\\texttt{line-color: \#e892a2}\\\texttt{line-width: 3}};
        \node[layer, fill=roleData!25, below=0.5cm of l0] (l1) {line\\\texttt{filter: [\mlop{"get"}, \mlval{"class"}] == \mlval{"primary"}}\\\texttt{line-color: \#fcd6a4}\\\texttt{line-width: 2}};
        \node[layer, fill=roleMir!30, below=0.5cm of l1] (l2) {line\\\texttt{filter: [\mlop{"get"}, \mlval{"class"}] == \mlval{"secondary"}}\\\texttt{line-color: \#f7fabf}\\\texttt{line-width: 1.5}};
        ...
        % Output (right)
        \node[merged, fill=roleGen!20, right=1.4cm of p2, text width=6.0cm] (out) {%
...
\begin{itemize}
    ...
    For example, if no style layer allows for a line with \texttt{class=ferry}}, the pass can remove features with that value.
