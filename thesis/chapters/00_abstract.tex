This thesis investigates automatic, joint style and data optimisation for MapLibre-based map rendering pipelines, asking whether composing style-level and data-level transformations yields additive or synergistic gains.
...
An evaluation across 15~styles, 18~scenarios, and 17~cumulative ablation steps demonstrates that tile shaving dominates load-time improvements, achieving \qtyrange{71}{75}{\percent} reductions (bootstrap \qty{95}{\percent} CIs $\leq$\qty{1}{pp} wide) for the two styles benchmarked end-to-end (Fiord and Liberty; the remaining styles are evaluated at the style level only, with tile-shaving effectiveness varying \qtyrange{19.8}{46.3}{\percent} across styles).
...
The two levels compose additively rather than synergistically at the aggregate level, though per-scenario synergy is present in a minority of style-scenario pairs dominated by low-zoom overview tiles.
