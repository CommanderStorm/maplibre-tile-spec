...
The optimiser is organised as a pipeline of passes at three levels of abstraction:
...
\begin{enumerate}
    ...
    \item \textbf{Structural passes} (\cref{sub_structural_optimisations}) deserialise the style into typed Rust structures generated from the upstream MapLibre specification (\cref{sub_codegen}) and apply transformations that require cross-layer or cross-source reasoning -- dead layer elimination, metadata refinement, layer merging, and cleanup.
...
Due to oddities how the maplibre style specification is specified, a simpler version was not possible.
...
This metadata is critical for structural passes.
Layer merging queries the MIR to determine whether a differing paint property supports feature-driven expressions (and can therefore be wrapped in a \texttt{case}/\texttt{match} over the filter discriminator) or is camera-only (and must be uniform across merged layers).
