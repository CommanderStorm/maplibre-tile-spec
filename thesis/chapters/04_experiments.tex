...
This cascade --- from style-level dead elimination and metadata refinement through advisory shrinkage into smaller tiles --- is the thesis's central architectural finding, quantified in \cref{sec_exp_mlt}.
...
\begin{enumerate}
    ...
    Furthermore, safety-critical libraries such as FastPFor~\cite{fastpfor} explicitly disclaim input validation, necessitating the multi-layer correctness validation described in \cref{sub_data_optimisations}.
\end{enumerate}
...
The central finding of this experiment is the cascade from style-level optimisations into data-level savings: dead layer elimination and metadata refinement (\cref{sec_exp_structural}) reduce the pruning advisory, which in turn reduces the data that must be encoded.
...
After shaving has removed unused columns and features, the remaining data has different statistical properties: fewer distinct property values produce longer \ac{RLE} runs (detected by \texttt{DataProfile::rle\_is\_viable} in \texttt{encoder/\allowbreak stream/\allowbreak optimizer.rs}), smaller value ranges improve delta encoding (detected by \texttt{delta\_is\_beneficial}), and smaller string vocabularies shrink dictionaries and improve \ac{FSST} compression ratios (probed by \texttt{fsst\_try\_train} in \texttt{encoder/\allowbreak property/\allowbreak strings.rs}).
