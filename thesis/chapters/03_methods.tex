...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[corpus, below right=1cm of mir] (test) {Render tests};

        % tanlged arrows
        \draw[query] (corpus.north -| serial.south) -- node[right, label] {validate} (mir);
        \draw[query] (opt) -- node[right, label, near start] {optimise} (serial);
        \draw[query] (scan) -- node[above, label] {tile statistics} (opt);
        \draw[arrow] (serial) -- node[above, near end, label] {sample styles} (test);
