...
The compiler analogues we surveyed (peephole rewriting, constant folding, fixpoint iteration) drive the expression-level passes that simplify individual style expressions without schema knowledge.
Predicate pushdown and selectivity estimation, the query-optimisation concepts from above, shape the structural passes and the pruning advisory that bridges style and data: the advisory is a pushdown plan derived from the optimised style, analogous to a query planner projecting only the columns a query reads.
And the encoding techniques covered last - columnar storage, integer coding, \ac{FSST}, and space-filling curves - feed into the data-level passes that re-encode shaved tiles with automatic strategy selection.
