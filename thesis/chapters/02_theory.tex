...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (s4) -- node[right, font=\small] {style}   (s4 |- renderer.south);
        \draw[arrow] (s3) -- node[right, font=\small] {tiles}   (s3 |- renderer.south);
        \draw[arrow] (s1) -- node[right, font=\small] {sprites} (s1 |- renderer.south);
        \draw[arrow] (s2) -- node[right, font=\small] {fonts}   (s2 |- renderer.south);

        % ── "over the network" brace ──
        \draw[decorate, decoration={brace, amplitude=6pt, mirror}, thick]
            ([xshift=4pt]renderer.south east) -- ([xshift=4pt]server.north east)
            node[midway, right=10pt, font=\scriptsize\itshape, text=gray] {over the network};

        % ── Data Source (bottom) ──
        \node[datasource, below=1.2cm of server] (datasource) {Data Source};

        \draw[arrow] (datasource.north) -- node[right, label] {convert to serving-friendly representation} (server.south);
    \end{tikzpicture}
