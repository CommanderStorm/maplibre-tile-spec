...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.38\linewidth}
        ...
        \begin{tikzpicture}[
            ...
            \node[decode, below=of u8, minimum width=3.2cm, align=left]     (name)
                {\texttt{enum <Name> \{}\\[1pt]\quad\scriptsize\texttt{Raw(Raw<Name>),}\\[0pt]\quad\scriptsize\texttt{Parsed(Parsed<Name>),}\\[1pt]\texttt{\}}};
            \node[decode, below=of name]   (tile)   {Tile*};

            \draw[arrow] (u8)   -- node[right, lbl, align=left] {zero-copy parse\\lazy decoding} (name);
            \draw[arrow] (name) -- node[right, lbl, align=left] {convert to\\row format} (tile);
        \end{tikzpicture}
    ...
    \begin{subfigure}[t]{0.38\linewidth}
        ...
        \begin{tikzpicture}[
            ...
            % *Encoder mediates Staged* -> Encoded*
            \node[enc, right=1.2cm of $(encoded)!0.5!(staged)$] (encnode) {*Encoder};

            \draw[arrow] (tile)    -- node[left, lbl, align=right] {convert to\\columnar} (staged);
            \draw[arrow] (staged)  -- node[left, lbl, align=right] {fully encode to\\wire-ready format} (encoded);
            \draw[arrow] (encoded) -- node[left, lbl] {zero-copy} (write);

            % Encoder arrows
            \draw[-{Stealth[length=2.5mm]}, thick, densely dashed]
                (staged.east) -- ++(0.3,0) |- (encnode.south);
            \draw[-{Stealth[length=2.5mm]}, thick, densely dashed]
                (encnode.north) -| ($(encoded.east)+(0.3,0)$) -- (encoded.east);
        \end{tikzpicture}
