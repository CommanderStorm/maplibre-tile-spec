...
WASM execution incurs overhead from ahead-of-time compilation, the absence of SIMD intrinsics (which \ac{FastPFOR} relies on natively), and JavaScript-to-WASM boundary crossings.
As a result, the columnar decoder's advantage materialises primarily on larger, feature-dense tiles where the per-column setup cost is amortised; on smaller tiles the per-column dispatch overhead narrows the gap to approximate parity with \ac{MVT}'s single-path protobuf parser.
Work is underway to shift more decoding work into WASM to reduce boundary-crossing overhead.
...
Taken together, the cost-control mechanisms are estimated to increase preprocessing cost compared to a fixed-plan encoder by roughly a factor of three to five, based on the configuration-space size (four sort strategies and two-to-four viable per-stream candidates after profiling); an empirical measurement of this overhead is left for future work alongside the per-pass cost breakdown below.
