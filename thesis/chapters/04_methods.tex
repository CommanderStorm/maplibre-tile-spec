...
Each pass preserves the visual-equivalence constraint formalised in \cref{sec_problem_statement}; together, the passes seek to minimise $c(S', T')$ across the metrics defined in \cref{sec_benchmarking}.
...
\begin{enumerate}
    ...
    Expression passes run to a fixpoint.
    The pass set is applied repeatedly (up to eight iterations) until no further changes are detected.
    ...
    \item \textbf{Data passes} apply changes to the served data, such as applying pruning advisorys or converting \ac{MLT} to \ac{MVT}. 
\end{enumerate}

The separation into three levels is deliberate and represents a specific resolution of the \emph{phase ordering problem}~\cite{clickCombiningAnalysesCombining1995}. 

Expression passes run first so that structural passes operate on already-simplified expressions, and dead elimination precedes merging so that gaps left by removed layers expose new merging opportunities.
Expression passes operate on raw JSON and require no knowledge of the style schema beyond expression syntax.
This permits fine-grained, composable rules that each match a local pattern and apply a rewrite.
The rules need not be aware of one another, and new rules can be added without modifying existing ones.
...
A cap of eight iterations provides a termination guarantee.
In practice, most styles converge within two to three iterations, and no style in either the benchmarking corpus or the Maputnik corpus has ever reached the cap.
Such fast convergance is likely because styles written by humans usually written to achive a point and there are only so many ways of doing this.

Structural passes, by contrast, require typed access to the full style document.
For example, dead layer elimination must resolve source-layer references across layers and sources, and layer merging must compare paint properties across layers.
