...
The map rendering architecture that defines what we are optimising, the compiler techniques whose analogues drive our expression-level passes, the query optimisation concepts that inform our filter rewriting and predicate reordering, and the columnar encoding strategies behind our data-level passes.
...
\begin{itemize}
\item \begin{sloppypar}\textbf{SVG} is a vector graphics format that can be used to render maps as shown by the ID Editor\footnote{\url{https://github.com/openstreetmap/iD}, last accessed 2026-03-16} or Leaflet\footnote{\url{https://leafletjs.com/}, last accessed 2026-03-16}.
...
OpenLayers\footnote{\url{https://openlayers.org/}, last accessed 2026-04-21} uses Canvas~2D as its primary renderer, rasterising vector features on the CPU rather than submitting them to the GPU.
...
\item \begin{sloppypar}\textbf{Low-level 3D Graphics APIs} like WebGL\footnote{\url{https://get.webgl.org/}, last accessed 2026-03-16}, WebGPU~\cite{WebGPU}, OpenGL\footnote{\url{https://registry.khronos.org/OpenGL-Refpages/gl4/}, last accessed 2026-03-16}, Metal\footnote{\url{https://developer.apple.com/metal/}, last accessed 2026-03-16}, Vulkan\footnote{\url{https://www.vulkan.org/}, last accessed 2026-03-16}, DirectX\footnote{\url{https://microsoft.github.io/DirectX-Specs/}, last accessed 2026-03-16}, etc.\ for rendering 3D graphics as shown by mapbox\footnote{\url{https://www.mapbox.com/}, last accessed 2026-03-16}, maplibre\footnote{\url{https://maplibre.org/}, last accessed 2026-03-16}, deck.gl\footnote{\url{https://deck.gl/}, last accessed 2026-03-16}, here\footnote{\url{https://www.here.com}, last accessed 2026-03-16} and many more.
...
\begin{itemize}
    ...
    \item \textbf{Analysis-focused formats} such as FlatGeobuf\footnote{\url{http://flatgeobuf.org/}, last accessed 2026-05-11}, GeoParquet\footnote{\url{https://geoparquet.org/}, last accessed 2026-05-11}, and GeoArrow are optimised for analytical queries (filtering, aggregation, joins) rather than tile-based rendering.
    ...
    It uses \acp{PBF}\footnote{\url{https://protobuf.dev/}, last accessed 2026-04-21} as its wire format and stores features in a \emph{row-oriented} layout:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \begin{sloppypar}\textbf{Client-side declarative} styles, as used by MapLibre\footnote{\url{https://maplibre.org/}, last accessed 2026-03-16} and Mapbox\footnote{\url{https://www.mapbox.com/}, last accessed 2026-03-16}, transmit a machine-readable style document to the client, which interprets the style at render time.
...
The MapLibre style specification\footnote{\url{https://www.maplibre.org/maplibre-style-spec/}, last accessed 2025-10-24} defines a map's appearance as a JSON document with the following key components:
...
The \textbf{Z-order (Morton) curve} interleaves the binary representations of the $x$ and $y$ coordinates to form a single index.
...
Three theoretical traditions map onto the three levels of the pipeline: compiler optimisation (peephole rewriting, constant folding, fixpoint iteration) drives the expression-level passes that simplify each style's JSON expressions without schema knowledge; query optimisation (projection pushdown, selectivity estimation) shapes the structural passes and the pruning advisory that bridges style and data; and the encoding techniques surveyed last - columnar storage, integer coding, \ac{FSST}, and space-filling curves - feed the data-level passes that re-encode shaved tiles with automatic strategy selection.
...
The following chapter formalises the joint optimisation problem and turns these foundations into concrete passes and an evaluation methodology.
\end{chaptersummary}
