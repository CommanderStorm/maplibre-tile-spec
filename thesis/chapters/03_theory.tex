...
The gap identified in \cref{ch_related_work} is that no existing tool composes style rewrites with data-level pruning into a joint pipeline.
...
\cref{sec_compiler_optimisations} surveys compiler optimisation techniques - peephole rewriting, constant folding, and fixpoint iteration - whose analogues are applied to style expressions.
\cref{sec_query_optimisations} presents query optimisation concepts - predicate pushdown and selectivity estimation - used for filter rewriting and reordering.
\cref{sec_data_encoding} covers the columnar encoding and compression schemes employed at the tile level.
\cref{sec_benchmarking} covers benchmarking methodology.
...
\textcircled{\scriptsize a}--\textcircled{\scriptsize e}.

Every optimisation in \cref{ch_methods} attaches to one of these steps:
Merging layers reduces work for \textcircled{\scriptsize a}.
Pruning features server-side makes \textcircled{\scriptsize b} not handle them.
Tightening \texttt{minzoom}/\texttt{maxzoom} makes the
\textcircled{\scriptsize c} guard true earlier.
Pre-folding zoom-dependent constants reduces the work in \textcircled{\scriptsize d} while
filter-to-property propagation simplifies the per-feature work inside
\textcircled{\scriptsize e}.
...
// (*@\textcircled{\scriptsize a}@*) Iterate source layers
...
for (const sourceLayerId in layerFamilies) {
    ...
    // (*@\textcircled{\scriptsize b}@*) Collect raw features
    const features = [];
    ...
    for (const family of layerFamilies[sourceLayerId]) {
        ...
        // (*@\textcircled{\scriptsize c}@*) Skip hidden layers
        if (layer.isHidden(this.zoom, true)) continue;

        // (*@\textcircled{\scriptsize d}@*) Recalculate zoom-dependent expressions
        recalculateLayers(family, this.zoom, availableImages);

        const bucket = buckets[layer.id] = layer.createBucket({...});

        // (*@\textcircled{\scriptsize e}@*) Filter features and build vertex data
...
\section{Compiler Optimisation Techniques}\label{sec_compiler_optimisations}
...
The standard \ac{TRS} notions of confluence (whether all rule-application sequences converge to the same normal form) and termination (whether all sequences reach a normal form) are revisited in the context of the concrete rewrite system in \cref{sub_expression_optimisations}.
...
\subsection{Profile-Guided Optimisation}\label{sub_pgo}
...
\section{Query Optimisation}\label{sec_query_optimisations}
...
A joint-distribution estimator, such as the one proposed in \cref{sec_seletivity_reordering} would therefore give tighter bounds, but the selectivity signal used in this thesis is consumed only by a \emph{reordering} heuristic - not by a cost-based plan chooser - and reordering is monotone in the ordering of the individual selectivities rather than in their absolute values.
...
\subsection{Materialized Views}\label{sub_materialized_views}
...
The future direction of serving-time applicability (\cref{sec_conclusion_future_work}) - applying the advisory as a per-request filter rather than a batch operation - is a form of virtual (non-materialized) view evaluation.

\subsection{Partial Evaluation and Superoptimization}\label{sub_partial_eval}
...
Filter-to-property constant propagation (\cref{sub_structural_optimisations}) is a textbook instance:
...
The encoding strategy competition (\cref{sub_encoding_strategies}) shares the spirit of \emph{superoptimization}~\cite{massalinSuperoptimizerLookingShortest1987} in that it exhaustively evaluates all candidate strategies over the actual data and retains the smallest result, rather than using profile data to guide a heuristic decision as in classical \ac{PGO} (\cref{sub_pgo}).
...
This section surveys the encoding and compression techniques used in the data-level optimisations of \cref{ch_methods}.
...
The encoding competition in \cref{sub_encoding_strategies} takes an exhaustive rather than cost-model approach, trialling a pruned set of candidate encodings and retaining the smallest output.
...
The bootstrap itself - a resampling method for constructing confidence intervals without distributional assumptions - was formalised by Efron and Tibshirani~\cite{efronIntroductionBootstrap1993} and is the specific statistical technique applied throughout this thesis's evaluation (\cref{ch_experiments}).
Hoefler and Belli~\cite{hoeflerScientificBenchmarkingParallel2015} codified twelve common pitfalls in reporting performance results; the evaluation in \cref{ch_experiments} follows their guidelines by reporting per-metric confidence intervals and separating throughput from latency measurements.
