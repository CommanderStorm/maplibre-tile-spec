...
All styles converged within two to three fixpoint iterations; no style in either corpus reached the eight-iteration cap.
...
No isolated A/B measurement was performed; the pass is retained because it is cheap to apply and cannot worsen correctness.
...
The headline merging numbers above are therefore reductions in the non-symbol pool only, and styles dominated by labels see proportionally smaller layer-count gains.

\paragraph{Downstream interaction with tile shaving}
Dead layer elimination has a second-order effect beyond reducing per-frame draw calls: each eliminated layer removes its property references from the tile pruning advisory computed in the data-level pipeline (\cref{sub:tile_shaving}).
...
Experiments~1 and~2 optimised the \emph{program} the renderer executes; this experiment asks whether the \emph{data} it consumes can be optimised using the same style information, and to what extent the two levels interact - the joint-optimisation question at the heart of Research Goal~R1.
The data-level pipeline applied here (\cref{sub:data_optimisations}) takes the optimised style from Experiments~1-2 and uses it to drive the tile pruning advisory and the encoding strategy competition that follows.

\textit{This experiment was done in collaboration with Rivian Automotive, Inc to optimise performance, correctness and software architecture of the encoder-decoder pair. The encoder and optimisation routine was fully implemented by us. Due to the collaboration, encoder performance is not discussed, even though it is better than the previous encoder/decoder pairs.}
...
\begin{enumerate}
    ...
    \item \textbf{Style + shaving}: the optimised style driving tile shaving - the combined configuration where dead layer elimination and metadata refinement may narrow the advisory relative to the unoptimised style.
    \item \textbf{Style + shaving + \ac{MLT} encoding}: the full pipeline - the encoding stage that delivers the shaved data to the wire via automatic encoding strategy selection (sort competition, per-stream profiling, string compression).
\end{enumerate}
...
For Fiord, the style-only reduction is \qty{14.1}{\percent} and shaving-only contributes a negligible reduction, yet the combined style+shaving reduction is \qty{14.7}{\percent} - a modest additional \qty{0.6}{pp} from dead-layer elimination narrowing the pruning advisory.
...
The native Rust micro-benchmarks show \numrange{4.6}{8.3}$\times$ higher decode throughput for \ac{MLT} over \ac{MVT}+gzip (\cref{tab:decode_throughput}), but this advantage does not fully transfer to the browser, where the \ac{MLT} decoder runs as a WASM module.
The dominant bottleneck is the WASM-to-JavaScript boundary: each decoded column must be transferred from WASM linear memory into JavaScript-visible typed arrays, incurring per-column copying and GC pressure that does not exist in a pure-native pipeline.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/tile_shave_per_style}
...
The distribution, summarised in \cref{tab:cross_style_summary}, fans out considerably: raw reductions range from \qty{10.6}{\percent} (AWS Hybrid, AWS Standard) to \qty{71.2}{\percent} (LocationIQ Streets), with a median of \qty{44.5}{\percent} and a standard deviation of \qty{19.4}{\percent}.
...
\Cref{tab:per_style_summary} summarises the per-style baseline and optimised gzip sizes, percentage reduction, and median changes in load time and FPS across the deep corpus.
...
\Cref{tab:marginal_contribution} reports the per-step deltas in raw size, gzip, Brotli, load time, and layer count: not all passes contribute equally, and the marginal contribution of each pass depends on the metric under consideration.
