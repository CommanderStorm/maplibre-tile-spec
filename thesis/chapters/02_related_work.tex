...
Style-level optimisation has attracted comparatively little research attention, and the closest precedents come from outside the map domain entirely.
...
\cref{sec:rw_style_optimisations} for style-level work and its CSS-minification cousins,
...
From a cartographic perspective, we understand style-driven tile shaving as a form of \emph{model generalisation}~\cite{mcmasterGeneralizationDigitalCartography1992}.
...
Our optimisations operate downstream, on the data and style that emerge from such pipelines, and do not require re-running the generator.
...
Our contribution is to lift these DB-internal optimisations across the network boundary between the tile server and the MapLibre renderer, treating the declarative style as a readable query plan.
...
We also include application-specific optimisations like spatial sort-order dimension (ID, Hilbert, Morton) that database encoders do not consider.
...
This ecosystem tooling demonstrates that individual style optimisations are recognised as valuable by practitioners, but no existing tool encomposes multiple optimisations passes.
...
We complement them by introducing a cumulative ablation methodology that isolates the contribution of each optimisations pass within a single renderer (MapLibre GL~JS).
...
Mapbox's closed-source \enquote{style-optimised tiles} service is the only precedent that genuinely couples style and data, and its terms of service explicitly preclude the independent evaluation needed to confirm what it actually does.
The style-side work is sparser still: Stamen's default-stripping utility and a handful of validators cover individual optimisations, but no published tool composes multiple passes, and the closest academic analogue (CSS minification) lives in a different domain altogether.
...
No prior work composes style and data optimisations into a single open pipeline: the data-side tools ignore the style, the only commercial precedent is closed-source, and the encoding format's strategy selection is style-blind.
Our contribution is the joint $(S, T) \to (S', T')$ optimisation formalised in \cref{sec:problem_statement}, together with a cumulative ablation methodology that measures whether the composition is additive or synergistic.
\cref{ch:experiments} quantifies the payoff - load times down by up to \qty{75}{\percent} and tile volume by \qty{28}{\percent} on the styles we benchmarked end-to-end - and the OGC API-Tiles standard~\cite{OGCAPITiles} keeps these gains deployable without vendor lock-in.

The next chapter introduces the foundations these techniques draw on: the rendering architecture, the compiler analogues behind the expression passes, the query-optimisation concepts behind filter rewriting and selectivity, and the columnar encoding strategies for the data-level passes.
\end{chaptersummary}
