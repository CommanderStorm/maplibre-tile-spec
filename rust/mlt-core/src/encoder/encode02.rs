...
use crate::decoder::{
    ColumnType02, DataType02, DictionaryType, LogicalEncoding, PhysicalEncoding, Presence02,
...
fn write_id02(id: StagedId, enc: &mut Encoder, codecs: &mut Codecs) -> MltResult<()> {
    ...
    use Presence02::{AllPresent, Inline};
    ...
    match id {
        ...
        StagedId::OptU32(v) => {
            begin_col02(enc, Inline, DT::Id, None)?;
        ...
        StagedId::OptU64(v) => {
            begin_col02(enc, Inline, DT::LongId, None)?;
            write_opt_col02(enc, &v.presence, |enc| {
                codecs.write_int_stream(&v.values, &ctx, enc)
            })
        }
...
fn write_prop02(prop: &StagedProperty, enc: &mut Encoder, codecs: &mut Codecs) -> MltResult<()> {
    ...
    macro_rules! opt_scalar {
        ($ct:ident, $v:expr) => {{
            begin_col02(enc, Inline, DT::$ct, Some(&$v.name))?;
            write_opt_col02(enc, &$v.presence, |enc| {
                codecs.write_int_stream(&$v.values, &StreamCtx::prop_data(&$v.name), enc)
            })
        }};
    ...
    match prop {
        ...
        D::OptBool(v) => {
            begin_col02(enc, Inline, DT::Bool, Some(&v.name))?;
        ...
        D::OptF32(v) => {
            begin_col02(enc, Inline, DT::F32, Some(&v.name))?;
            write_opt_col02(enc, &v.presence, |enc| {
                codecs.write_float_stream(&v.values, StreamType::Data(DictionaryType::None), enc)
            })
        }
        ...
        D::OptF64(v) => {
            begin_col02(enc, Inline, DT::F64, Some(&v.name))?;
            write_opt_col02(enc, &v.presence, |enc| {
                codecs.write_float_stream(&v.values, StreamType::Data(DictionaryType::None), enc)
            })
        }
