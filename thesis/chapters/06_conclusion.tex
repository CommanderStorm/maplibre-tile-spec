...
The fourth is the empirical \textbf{evaluation} itself (\cref{ch:experiments}), which confirms that \textbf{the three optimisation categories are complementary rather than redundant}.
...
Stepping back from the numbers, the compiler-optimisation framing - peephole rewriting, fixpoint iteration, dead code elimination - turns out to transfer cleanly to the map-style domain.
...
The cumulative ablation in \cref{sub:combined_ablation} and the per-style results in \cref{sec:app_per_style} rank the passes by marginal contribution and group them into three clusters with quite different behaviour.
...
Within the structural family, dead elimination and layer merging dominate because they reduce the per-frame draw-call count directly; the two are strongly synergistic, because dead elimination removes gaps between mergeable layers that would otherwise prevent the merge pass from recognising adjacency.
Data-level optimisations operate on a different axis from both: tile shaving reduces transfer size by using the optimised style's property and value references to build a pruning advisory, and the \ac{MLT} encoder's sort-strategy and per-stream competition reduces encoded size on top of that.
...
The style optimiser already depends only on the style and can run at deployment time; the pruning advisory could plausibly run as a lightweight serving-time filter built on column selection and predicate evaluation, in the spirit of adaptive query processing~\cite{deshpandeAdaptiveQueryProcessing2006}.
...
The end-point of this trajectory is direct integration of the optimised \ac{MLT} decoder into MapLibre GL~JS, which is the next concrete piece of work we plan to take on.
