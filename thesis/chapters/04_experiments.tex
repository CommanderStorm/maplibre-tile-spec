...
This experiment addresses the complementary question: can the \emph{data} itself be transformed to reduce transfer size and decoding cost, and do style-level and data-level optimisations compose?

The research by Tremmel et al.~\cite{tremmelMapLibreTileNext2025} has demonstrated that the \ac{MLT} format is structurally superior to the protobuf-based \ac{MVT} format, offering columnar storage, built-in compression, and a richer type system.
...
\cref{sub_data_optimisations} provides a high-level overview of the data-level techniques; this section details the algorithms and their experimental configuration.
The data-level optimisations pipeline operates in three stages that mirror the structure of a database query optimiser: \emph{logical} rewriting (what data to keep), \emph{physical} strategy selection (how to encode it), and \emph{cost-based} competition (which encoding is smallest).
...
\noindent Stage~1 is the key \emph{style-data co-optimisations}: the style determines what data survives, and the style-level optimisations from Experiments~1-2 cascade into smaller advisories (fewer live layers means fewer referenced properties) that produce smaller tiles.
Stage~2 uses data statistics to improve compressibility.
Stage~3 is a data-only optimisations that adapts encoding to the statistical properties of the data after pruning and transformation.
...
The style optimisations from Experiments~1-2 thus \emph{cascade} into data-level savings.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Style optimisations
        \node[style] (sopt) {Style optimisations\\{\scriptsize (Experiments 1-2)}};
...
The central evaluation question for this experiment is whether style-level and data-level optimisations \emph{compose} - that is, whether optimising the style first, then encoding the resulting tile set, produces better results than either optimisations category in isolation.
This directly addresses Research Goal~R1 (to what extent does data-style co-optimisations reduce tile size and improve rendering).

The evaluation uses a cumulative design with five configurations, each adding one optimisations layer:
...
\noindent Configurations~2-4 isolate the interaction effect: if the combined reduction of style+shaving exceeds the sum of style-only and shaving-only, the two optimisations categories are \emph{synergistic}; if less, they are partially redundant.
...
\begin{figure}[htbp]
    ...
    \caption{Total tile size per zoom level across configurations~1-5.  The gap between \enquote{style-only} (config~2) and \enquote{style+shaving} (config~4) quantifies the cascade effect; the gap between config~4 and config~5 quantifies the contribution of MLT encoding optimisations}
...
The most significant finding of this experiment is whether style-level and data-level optimisations are synergistic.
...
This cascade - from style simplification through advisory shrinkage to tile reduction - is the concrete realisation of the \enquote{data-style co-optimisations} promised in the thesis title.
...
\subsection{Decoding Throughput}\label{sub_decode_throughput}
...
\bottomrule
\end{tblr}
\caption{Full-decode latency for \ac{MLT}-Rust vs.\ \ac{MVT} on OpenMapTiles fixture tiles, measured by Criterion micro-benchmarks (\texttt{decoding\_e2e.rs}, 100~samples per cell).
