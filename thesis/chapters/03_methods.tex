...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % tanlged arrows
        \draw[red,->] (corpus) -- node[red,below, label] {validate} (serial);
        \draw[query] (opt) -- node[right, label] {optimise} (serial);
        \draw[query] (scan) -- node[above, near start, label] {statistics} (opt);
        \draw[red,->] (serial) -- node[red,right, label] {sample styles} (test);
        \draw[red,->] (test) -- node[red,right, label] {fuzz / validate} (opt);
    \end{tikzpicture}
