...
With the benchmark values ($r \approx \qty{3}{\milli\second}$ per tile from the load-time reduction, $C_f \approx \qty{26}{\milli\second}$ under the Rusan et al.~\cite{rusanAssessmentPacketLatency2015} $\frac{\text{size}}{30\frac{Mb}{s}} + 25ms$ throttle asssuming a \qty{50}{\kilo\byte} average tile), shaving breaks even at roughly $S \leq 5$ canonical styles; beyond that, the hybrid strategy - shaving canonical styles, serving raw \ac{MVT} for the tail - might be a better deployment pattern.
