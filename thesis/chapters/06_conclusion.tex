...
\begin{enumerate}[label=\textbf{C\arabic*}]
    ...
    Adaptive multi-strategy encoding in the \ac{MLT} Rust encoder (\cref{sub:data_optimizations}) reduces tile volume by \qty{28}{\percent} over the fixed-plan Java baseline.
    The advantage persists under every wire compressor tested (\cref{tab:mvt_mlt_comparison}).
    We find the cross-family interaction between style-level passes and tile shaving to be additive rather than synergistic (\cref{sub:mlt_discussion}).
    ...
    The Rust encoder alone accounts for \qty{12}{\percent}, with style-driven tile shaving providing the remainder.
...
\paragraph{Tile shaving and encoding dominate while style passes stack additively}
