...
Expression rewrites come first: a set of peephole rules - constant folding, algebraic simplification, stats-driven folding from tile statistics, and selectivity-based predicate reordering - is applied to the style's \ac{JSON} expressions and iterated to a fixpoint.
...
Our \ac{MLT} encoder reduces total tile volume by \qty{28}{\percent} over the reference encoder and \qty{12}{\percent} over gzip-compressed \ac{MVT} on the Germany OpenMapTiles tileset.
