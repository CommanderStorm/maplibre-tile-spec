...
\item Within the family, constant folding dominates; selectivity reordering is empirically unmeasured.
\end{keytakeaways}

In this experiment we evaluate the eight expression-level passes of \cref{sub:expression_optimisations} - the rewrites that act locally on each style's JSON expressions and run to a fixpoint.
...
Where the previous experiment rewrote local expressions, in this one we evaluate the passes that reason across layers and sources - the structural family of \cref{sub:structural_optimisations}.
...
\item The \ac{MLT} format alone is not the win - \ac{MLT}-Java drifts above $1.0\times$ \ac{MVT} under gzip/Brotli; the gain comes from the encoder's strategy competition.
\item The columnar decoder's native speedup (\numrange{4.6}{8.3}$\times$) does not survive the WASM/JS boundary in-browser.
\end{keytakeaways}

In \cref{sec:exp_structural,sec:exp_expressions} we optimised the \emph{program} the renderer executes; in this experiment we ask whether we can optimise the \emph{data} it consumes using the same style information, and to what extent the two levels interact - the joint-optimisation question at the heart of Research Goal~R1.
...
\item Initial style size does not predict reduction ($r = 0.16$, $n = 13$).
\item These numbers are a lower bound - stats-driven and data-level passes were not applied here.
\end{keytakeaways}

In the preceding experiments we evaluated optimisations effectiveness on a curated corpus of 15 styles.
...
In this section we present the full pipeline ablation - all 17 cumulative steps from baseline through tile rewriting - and analyse the interactions between optimisations categories.
