...
This is the closest commercial precedent for style-data co-optimizations, but it is opaque, closed-source, and \ac{API}-only - it cannot be extended, audited, or deployed independently.
...
\ac{MVT} has since been adopted as a conformance class in the \ac{OGC} API-Tiles standard~\cite{OGCAPITiles}, making it the baseline format for any standards-compliant tile delivery.
...
Hague et al.~\cite{hagueCSSMinificationConstraint2019} attack \ac{CSS} rule merging as a Max-SAT problem over a node-weighted bipartite graph of rules and properties, with ordering constraints from the cascade.
Their SatCSS tool beats six production \ac{CSS} minifiers on real benchmarks.
We are not the first to see the parallel - merging declarative style rules subject to ordering constraints is the same problem whether the order comes from the \ac{CSS} cascade or the painter's algorithm - but the formalisation does not appear to have crossed into the map-rendering literature.
...
Below 10\,000 features, \ac{SVG}-based renderers (Leaflet, OpenLayers) are faster; above 50\,000 features, \ac{WebGL}-based renderers (Mapbox GL~JS, MapLibre GL~JS) dominate due to \ac{GPU} acceleration.
...
The style-side work is sparser still: Stamen's default-stripping utility and a handful of validators cover individual optimizations, but no published tool composes multiple passes, and the closest academic analogue (\ac{CSS} minification) lives in a different domain altogether.
...
\cref{ch:experiments} quantifies the payoff - \textbf{load times down by \qtyrange{63}{74}{\percent} (median \qty{71}{\percent}) and tile volume by \qty{28}{\percent}} on the styles we benchmarked end-to-end - and the \ac{OGC} API-Tiles standard~\cite{OGCAPITiles} keeps these gains deployable without vendor lock-in.
