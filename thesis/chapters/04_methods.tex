...
The chapter opens with a formal statement of the optimization problem and its visual-equivalence constraint (\cref{sec:problem_statement}), then delimits the work in \cref{sec:scope} - naming MapLibre GL~JS as the target system and listing the concerns deliberately out of scope - and proceeds level by level through the three-level pipeline (\cref{sec:approach}) and the individual passes that implement it.
...
As discussed in \cref{sec:styles}, performance research on interactive vector maps is concentrated on Mapbox and MapLibre styles; the Mapbox codebase is proprietary and may not be evaluated without explicit consent, leaving MapLibre as the only viable open research target.
...
\begin{itemize}
    \item \emph{Energy measurement.} Energy effects are discussed only qualitatively; we report no \ac{RAPL} or battery deltas because the benchmark host is a desktop workstation rather than a mobile device (\cref{sub:metrics}).
    \item \emph{Lossy geometry transformations.} Coordinate-precision reduction, Visvalingam--Whyatt, Douglas--Peucker, and polygon coalescing would violate the pixel-identity contract of \cref{eq:visual_equivalence} and are deferred to future work; we therefore quantify the size reductions achievable \emph{without} touching geometry, understating the headroom that composition with an upstream simplifier could reach.
    \item \emph{Symbol-layer merging.} MapLibre evaluates symbol placement per layer through a tile-wide \texttt{CollisionIndex}, so merging symbol layers would alter label suppression order; the detailed argument is given alongside the structural passes (\cref{sub:structural_optimizations}).
    ...
    \item \emph{Tile schemas other than \ac{OMT}.} Non-OMT vector tile sources are excluded from the deep-corpus evaluation, and the stats-driven thresholds used by the pipeline are tuned on Germany~OMT only (\cref{sec:gathering_sample}).
    \item \emph{Online and incremental serving.} The optimizer is an offline, one-shot rebuild over a (style, tile~set) pair; no integration with a live tile server is assumed, and the optimized artefacts can be served by any standards-compliant tile infrastructure.
    \item \emph{The reference Java encoder.} The legacy self-contained Java \ac{MLT} encoder of~\cite{tremmelMapLibreTileNext2025b} is retained as a baseline for the encoding experiments of \cref{ch:experiments}; it is not a target of optimization in this thesis.
\end{itemize}
...
Under the soundness contract of \cref{sec:scope}, the passes from~\cref{sub:pipeline} together seek to minimize $c(S', T')$ across the metrics defined in \cref{sec:benchmarking}.
...
Hand-written Rust types are not a viable alternative: MapLibre publishes the specification in its own JSON-based \ac{DSL}, and a hand-maintained shadow would require manual synchronisation with every specification change and risk silent divergence.
...
The style optimizer is a standalone Rust binary; the tile re-encoder described in \cref{sub:data_optimizations} shares the same Cargo workspace and is split into a core encoder/decoder crate and a Java wrapper crate so that a \ac{JVM}-based tile-generation pipeline can consume the encoder without re-implementing it.
...
The Java wrapper exposes the generated bindings through the Panama \ac{FFM} \ac{API}\footnote{\url{https://openjdk.org/jeps/454}, last accessed 2026-05-23} rather than conventional \ac{JNI} glue, eliminating hand-written C stubs and giving the \ac{JVM} direct, zero-copy access to off-heap buffers.
...
Absorption is sound because MapLibre filter operators are pure (the filter language exposes no feature-state mutation), so dropping an operand cannot discard an observable side effect.
...
["all",
	...
	\caption{Examples of expression-level transformations applied to representative style inputs. Each panel shows a before/after JSON example pair demonstrating how these optimisations interact.}
...
The merging algorithm proceeds in three phases, illustrated in \Cref{fig:layer_merge_example}.
...
Layer types that support sort keys (\texttt{\mlval{fill}}, \texttt{\mlval{line}}, \texttt{\mlval{circle}}, \texttt{\mlval{fill-extrusion}}) are candidates; \texttt{\mlval{symbol}} layers are excluded because MapLibre performs symbol placement layer by layer (the tile-wide \texttt{CollisionIndex} is queried iteratively, one symbol layer at a time, so layer ordering determines placement priority), and merging symbol layers would alter which labels can suppress each other, changing the rendered output.

\Cref{lst:placement} shows the per-layer processing; \stepcirc{1} constructs a fresh instance when a symbol layer's placement begins, and \stepcirc{2} tears it down before the next layer starts; the underlying \texttt{Placement} object (and its \texttt{CollisionIndex}) is shared across all layers via \texttt{this.placement}, which is what makes the \texttt{LayerPlacement} scope per-layer rather than global.
...
(3)~any MapLibre runtime behavior whose scope is the individual layer - per-layer symbol placement (which iterates the shared, tile-wide \texttt{CollisionIndex} one symbol layer at a time), \texttt{\mlop{feature-state}}, \texttt{\mlprop{*-translate}} and similar camera-only properties - must not have that scope widened by the merge.
...
We accept \texttt{\mlprop{fill-extrusion}} layers as merge candidates in principle, but the current implementation inherits the same \texttt{\{\mlprop{fill,line,circle}\}\mlprop{-sort-key}} restriction; in practice their relative ordering is dominated by the depth buffer rather than the painter's algorithm, so the renderer uses the \texttt{\{\mlprop{fill,line,circle}\}\mlprop{-sort-key}} property to break ties at co-planar surfaces only.
Invariant~(3) is guaranteed by the exclusion of symbol layers from the candidate set (which preserves per-layer placement ordering even though the underlying \texttt{CollisionIndex} is tile-wide), by the \texttt{expression.feature} gate (camera-only properties must be uniform and thus remain layer-scoped after the merge), and by refusing to merge layers with differing non-mergeable properties at all.
...
\begin{itemize}
    ...
    For example, if no style layer allows for a line with a \texttt{\mlval{class=ferry}} property, the pass can remove features with that value.
...
A statistics collection phase provides this knowledge by scanning the tile set and producing a per-source, per-source-layer statistical profile; the optimizer accepts either an MBTiles database, in which case the scan runs on demand, or a pre-computed statistics file produced by an earlier scan.
