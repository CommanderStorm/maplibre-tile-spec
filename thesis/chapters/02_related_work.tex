...
The literature relevant to closing that gap is uneven.
Tile data pruning has been attacked head-on by several open-source tools, each addressing a slice of the problem and none composing with style rewriting.
...
Style-level optimization has attracted comparatively little research attention, and the closest precedents come from outside the map domain entirely.
Benchmarking methodology, finally, is the area where the prior art is mature; we mostly borrow rather than contribute.
...
\cref{sec:rw_style_optimizations} for style-level work and its CSS-minification cousins,
...
The exact set of optimizations is not disclosed and given that MapBox does not have a open research stance (their terms of service explicitly forbid this), reverse-engineering would likely be illegal\footnote{\url{https://media.ccc.de/v/36c3-11089-reflections_on_the_new_reverse_engineering_law}}.
...
Tippecanoe\footnote{\url{https://github.com/felt/tippecanoe}, last accessed 2026-04-21} is the standard open-source tool for building vector tilesets from large GeoJSON or FlatGeobuf\footnote{\url{http://flatgeobuf.org/}, last accessed 2026-05-11} collections.
...
Tile data at different zoom levels is, in effect, a discrete multi-scale generalization hierarchy, though this thesis does not contribute new generalization algorithms.
...
The distinction is that classical generalization selects features by geometric criteria (area, length, density), whereas style-driven shaving selects by attribute criteria (which properties and values the style references).
...
The transformations we develop in \cref{ch:methods} -- formalised as the $(S, T) \to (S', T')$ problem of \cref{sec:problem_statement} -- that compute a tile pruning advisory from the style and apply it to the tile data are a form of \emph{projection pushdown}~\cite{graefeQueryEvaluationTechniques1993}.
...
Our tile shaving is a form of early materialisation (columns are removed at encoding time), while the selective column decoding we discuss as future work (\cref{sec:conclusion_future_work}) is a form of late materialisation (columns are skipped at decode time).
Modern query engines extend the pushdown with predicate filters~\cite{abadiDesignImplementationModern2013}.
Modern distributed query engines such as Spark SQL~\cite{armbrustSparkSQLRelational2015} implement predicate and projection pushdown as composable tree-transformation rules applied in batches by a rule-based optimizer - a direct descendant of the extensible rule-based query rewriting architecture introduced by Pirahesh et~al.~\cite{piraheshExtensibleRuleBased1992} in the Starburst system - an architecture structurally parallel to the three-phase pipeline we develop in this thesis.
Our contribution is to lift these DB-internal optimizations across the network boundary between the tile server and the MapLibre renderer, treating the declarative style as a readable query plan.
...
Tremmel et al.~\cite{tremmelMapLibreTileNext2025b} introduced the MapLibre Tile (MLT) format, a columnar alternative that achieves up to $3\times$ tile size reduction on encoded tilesets and up to $6\times$ on certain large tiles relative to \ac{MVT}.
...
Damme et~al.~\cite{dammeComprehensiveExperimentalSurvey2019} evaluated dozens of lightweight integer encodings and developed a cost-based selection strategy. Boissier~\cite{ganBaguaScalingDistributed2021} formulated encoding selection as a budget-constrained optimization problem.
...
Our trial-based competition is closest in spirit to the profile-guided approach, but substitutes exhaustive evaluation over a bounded candidate space for the learned or heuristic strategies used in database systems.
...
His work established the architectural vocabulary that \ac{MVT} and \ac{MLT} build on, but predates the shift to columnar encoding and does not consider style-awareness as a dimension of the encoding problem - precisely the dimension we add to the tile encoding pipeline.
...
\section{Style Optimisation}\label{sec:rw_style_optimizations}
...
Stamen Design published \texttt{mapbox-gl-style-remove-defaults}\footnote{\url{https://github.com/stamen/mapbox-gl-style-remove-defaults}, last accessed 2026-5-27}, a tool that removes properties set to their specification default values from Mapbox GL style JSON.
...
Hague et al.~\cite{hagueCSSMinificationConstraint2019} attack CSS rule merging as a Max-SAT problem over a node-weighted bipartite graph of rules and properties, with ordering constraints from the cascade.
Their SatCSS tool beats six production CSS minifiers on real benchmarks.
We are not the first to see the parallel - merging declarative style rules subject to ordering constraints is the same problem whether the order comes from the CSS cascade or the painter's algorithm - but the formalisation does not appear to have crossed into the map-rendering literature.
Our layer-merging pass (\cref{ch:methods}) takes a much simpler heuristic route than SatCSS does, since the per-tileset build budget rules out reaching for a SAT solver.

More broadly, rule-based optimization is a well-studied problem in both databases and compilers.
Pirahesh et~al.~\cite{piraheshExtensibleRuleBased1992} established the paradigm of \emph{rule-based query rewriting} as a distinct optimization phase in the Starburst system.
...
Our expression-level passes follow the same architecture: we apply declarative rewrite rules in priority order to style expressions, which makes Starburst the direct database ancestor of our approach.
The modern state of the art for avoiding rule-ordering sensitivity is \emph{equality saturation}~\cite{tateEqualitySaturationNew2009,willseyEggFastExtensible2021}: rather than applying rewrites sequentially and committing to the first match, an e-graph simultaneously represents all equivalent forms and selects the optimal one after saturation.
Our rewrite system is not confluent and applies rules in a fixed priority order; equality saturation would eliminate this ordering dependence at the cost of significantly higher implementation and runtime complexity.
Since the peephole rules are individually simple and the styles converge within a few fixpoint iterations (\cref{sub:fixpoint} provides the convergence analysis), we judge that the simpler sequential approach suffices for the current use case, but equality saturation remains a natural future direction if the rule set grows or ordering sensitivity becomes a practical concern.
...
The style-side work is sparser still: Stamen's default-stripping utility and a handful of validators cover individual optimizations, but no published tool composes multiple passes, and the closest academic analogue (CSS minification) lives in a different domain altogether.
Tremmel and Zink's \ac{MLT} format advances the encoding layer but picks strategies without consulting the style.
The benchmarking literature is in better shape; we adopt its statistical methodology rather than extending it.
...
Our contribution is the joint $(S, T) \to (S', T')$ optimization formalised in \cref{sec:problem_statement}, together with a cumulative ablation methodology that measures whether the composition is additive or synergistic.
\cref{ch:experiments} quantifies the payoff - \textbf{load times down by \qtyrange{63}{74}{\percent} (median \qty{71}{\percent}) and tile volume by \qty{28}{\percent}} on the styles we benchmarked end-to-end - and the OGC API-Tiles standard~\cite{OGCAPITiles} keeps these gains deployable without vendor lock-in.

The next chapter introduces the foundations these techniques draw on: the rendering architecture, the compiler analogues behind the expression passes, the query-optimization concepts behind filter rewriting and selectivity, and the columnar encoding strategies for the data-level passes.
\end{chaptersummary}
