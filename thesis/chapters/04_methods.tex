...
Let $\mathcal{I}$ be the set of rasterised images, and
\begin{equation}\label{eq:render_function}
    R : \mathcal{S} \times \mathcal{T} \times \mathcal{V} \to \mathcal{I}
\end{equation}
\noindent be MapLibre's deterministic rendering function.

The optimizer is a function $\mathcal{O}: \mathcal{S} \times \mathcal{T} \to \mathcal{S} \times \mathcal{T}$ with $(S', T') = \mathcal{O}(S, T)$.
Given a cost vector $c$ over the deployment-relevant metrics of \cref{sec:benchmarking}, $\mathcal{O}$ seeks to reduce the components of $c(S', T')$ under two formal constraints.
We do not aggregate the components into a scalar; we report each separately in the per-pass ablations.

The first constraint is \emph{visual equivalence}.
Writing $\equiv$ for pixel-identity,
...
\begin{equation}\label{eq:idempotency}
    \mathcal{O}(\mathcal{O}(S, T)) = \mathcal{O}(S, T) \quad \text{for all } (S, T) \in \mathcal{S} \times \mathcal{T}.
\end{equation}
Feeding $\mathcal{O}$'s output back as input yields the same pair, ruling out unbounded chained improvement by construction.
The expression-pass fixpoint of \cref{sub:fixpoint} enforces idempotency on the style half; the deterministic, statistics-driven encoding choices of \cref{sub:data_optimizations} enforce it on the tile half.
Both halves are exercised by the round-trip proptest and libFuzzer harness of \cref{par:encoder_validation}.
...
For example, the stats-driven folding pass cannot soundly remove a \mlop{match} arm without a full census of the tile data, because the arm may be reached by features the statistics have not observed; the pass is consequently gated on a sample rate of~1.0.
...
\subsection{Metadata Optimizations}\label{sub:metadata_optimizations}
...
\begin{itemize}
    ...
    \item \textbf{Optimization cost}: preprocessing time introduced by each pass, measured as wall-clock time on the optimizer's host machine; this is an offline cost amortised over all subsequent tile deliveries.
