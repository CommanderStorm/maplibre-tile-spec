...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.45\linewidth}
        ...
        \begin{tikzpicture}[
            ...
            \node[dec, right=2cm of $(u8)!0.5!(name)$] (decnode) {Decoder\\{\scriptsize reusable decoding buffers\\memory usage limits}};

            \draw[arrow] (u8)   -- node[left, lbl, align=right] {zero-copy parse;\\columns decoded\\lazily} (name);
            \draw[arrow] (name) -- node[left, lbl, align=right] {convert to\\row format} (tile);

            % Phantom to mirror Decoder so pipeline stays centred
            \node[inner sep=0pt, minimum size=0pt, left=2cm of $(u8)!0.5!(name)$] {};

            % Decoder arrows
            \draw[-Stealth, thick, densely dashed] (u8.east) -| (decnode.north);
            \draw[-Stealth, thick, densely dashed] (decnode.south) |- (name.east);
        \end{tikzpicture}
    ...
    \begin{subfigure}[t]{0.45\linewidth}
        ...
        \begin{tikzpicture}[
            ...
            \node[enc, left=2cm of $(encoded)!0.5!(staged)$] (encnode) {*Encoder};
            ...
            % Phantom to mirror *Encoder so pipeline stays centred
            \node[inner sep=0pt, minimum size=0pt, right=2cm of $(encoded)!0.5!(staged)$] {};
