...
Let $r$ denote the per-tile rendering-time saving from shaving (derived from the load-time reduction divided by the number of fetched tiles), let $S$ denote the number of distinct styles served, and let $C_f$ denote the origin fetch cost per cache miss (RTT plus transfer time).
...
With the benchmark values ($r \approx \qty{3}{\milli\second}$ per tile from the load-time reduction, $C_f \approx \qty{26}{\milli\second}$ under the Rusan et al.~\cite{rusanAssessmentPacketLatency2015} $\frac{\text{size}}{30\frac{Mb}{s}} + 25ms$ throttle asssuming a \num{50}{\kilo\byte} average tile), shaving breaks even at roughly $S \leq 5$ canonical styles; beyond that, the hybrid strategy - shaving canonical styles, serving raw \ac{MVT} for the tail - is the recommended deployment pattern.
