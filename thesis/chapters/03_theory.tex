...
\noindent Every optimisation in \cref{ch:methods} attaches to one of these steps:
...
\begin{itemize}
	...
	\begin{tabular}[t]{@{}rcl@{\qquad}rcl@{\qquad}rcl@{\qquad}rcl@{}}
		$x \land \mathit{true}$ & $=$ & $x$ &
		$x \lor \mathit{false}$ & $=$ & $x$ &
		$x + 0$                 & $=$ & $x$ &
		$x \cdot 1$             & $=$ & $x$
	\end{tabular}
	
	\item \emph{Annihilation}:
	\begin{tabular}[t]{@{}rcl@{\qquad}rcl@{\qquad}rcl@{}}
		$x \land \mathit{false}$ & $=$ & $\mathit{false}$ &
		$x \lor \mathit{true}$   & $=$ & $\mathit{true}$ &
		$x \cdot 0$              & $=$ & $0$
	\end{tabular}
	
	\item \emph{De~Morgan's laws}:
	\begin{tabular}[t]{@{}rcl@{\qquad}rcl@{}}
...
A \emph{materialized view} is a derived relation whose contents are precomputed from a query over base tables and stored for fast access~\cite{guptaMaintenanceMaterializedViews1999}.
