...
Although all of these resources can be optimised, tiles are usually the most critical due to their continuously streaming-int nature as a user moves the map.
They usualy have a cumulatively higher data volume due to this, even though their individual tiles may be much smaller.
All other resources depend on locale slightly (for example loading different Unicode ranges or fonts), but are generally only handled one time.
...
\begin{itemize}
    ...
    This is also known as "turtle style"\footnote{\url{https://docs.python.org/3/library/turtle.html}}.
...
Individual peephole rules may expose further optimisations opportunities: constant folding a subexpression may create a new constant parent, which in turn enables dead branch elimination.
...
The expression-level optimiser is, formally, a \emph{term rewriting system} (TRS): the style expressions are terms, and the peephole rules are rewrite rules applied modulo a traversal strategy~\cite{baaderTermRewritingAll1998}.
...
The standard TRS notions of confluence (whether all rule-application sequences converge to the same normal form) and termination (whether all sequences reach a normal form) are revisited in the context of the concrete rewrite system in \cref{sub_expression_optimisations}.
A closely parallel case study is Halide's term rewriting system: Newcomb et~al.~\cite{newcombVerifyingImprovingHalide2020} showed that Halide's DSL optimiser --- which, like this thesis's expression passes, applies a set of hand-written rewrite rules in a fixed order to simplify DSL expression trees --- suffers from the same confluence and termination concerns, and developed synthesis-based techniques to verify correctness and discover missing rules.
\end{sloppypar}


\subsection{Profile-Guided Optimisation}\label{sub_pgo}

\emph{Profile-guided optimisation} (PGO) uses runtime execution profiles to specialise a program for its observed workload~\cite{changUsingProfileInformation1991}.
...
Arnold et~al.~\cite{arnoldSurveyAdaptiveOptimization2005} survey how virtual machines extend PGO to an \emph{adaptive} setting, where profiles are collected and optimisations applied continuously at runtime.

The style optimiser performs an analogous form of PGO: tile-level statistics (value cardinalities, per-property frequency distributions, geometry-type counts) are the ``profile data,'' and the stats-driven folding pass uses this profile to specialise the style for the observed data distribution --- collapsing \texttt{match} arms whose values never occur in the tile census, tightening zoom bounds based on observed data ranges, and converting high-selectivity predicates into constants.
The key difference from classical PGO is that the profile is exact (the tile census covers every feature in the tileset at a sample rate of~1.0) rather than sampled, and the optimisation target is a declarative style document rather than machine code.
...
When the base data changes, the materialized view must be updated - either by full recomputation or by \emph{incremental view maintenance} (IVM), which propagates only the delta.
The cascade developed in this thesis has a direct structural analogue: the tile set is the base relation, the rendering style is the view definition, and the shaved tile is the materialized view.
...
Filter-to-property constant propagation (\cref{sub_structural_optimisations}) is a textbook instance: an equality constraint in the filter (static) is substituted into paint expressions, collapsing data-dependent branches that can never be reached given the known filter.

The encoding strategy competition (\cref{sub_encoding_strategies}) shares the spirit of \emph{superoptimization}~\cite{massalinSuperoptimizerLookingShortest1987} in that it exhaustively evaluates all candidate strategies over the actual data and retains the smallest result, rather than using profile data to guide a heuristic decision as in classical profile-guided optimisation (\cref{sub_pgo}).
...
Second, \emph{better compression}: values of the same type and semantic domain are stored together, exhibiting higher homogeneity and thus compressing more effectively than interleaved multi-type records.
...
The question of which encoding to apply to a given column has been studied systematically: Abadi et~al.~\cite{abadiIntegratingCompressionExecution2006} proposed property-based selection rules, and Damme et~al.~\cite{dammeComprehensiveExperimentalSurvey2019} developed a cost-model-driven selection strategy that picks the encoding with the lowest expected cost for a given data distribution.
The encoding competition in \cref{sub_encoding_strategies} takes an exhaustive rather than cost-model approach, trialling all candidate encodings and retaining the smallest output.
...
The estimate is unbiased with variance $O(1/k)$~\cite{broderResemblanceContainmentDocuments1998}.
...
\begin{itemize}
    \item \textbf{Power consumption} in Watts, which is relevant for mobile devices and battery-powered systems.  This thesis does not measure power consumption - no RAPL counters, battery deltas, or external power meters are used, as the benchmark host is a desktop workstation rather than a battery-constrained mobile device.  Power is listed here as a candidate metric because transfer-size reductions and reduced draw-call counts are established proxies for energy savings on mobile targets: cellular radio energy is proportional to data volume, and GPU active time scales with draw-call count.  No quantitative energy claim is made in this thesis; the qualitative energy discussion in \cref{ch_experiments} is limited to stating the expected direction of the effect.
