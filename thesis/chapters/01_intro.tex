...
A slow network stalls the first. A battery- or thermally-bound device stalls the second.
MapLibre GL~JS\footnote{\url{https://maplibre.org/maplibre-gl-js/docs/}, last accessed 2026-05-28} and Mapbox GL~JS\footnote{\url{https://www.mapbox.com/mapbox-gljs}, last accessed 2026-05-28} deliver these maps by streaming tiled geometry to the browser and letting a client-side style document drive a \ac{WebGL}- or \ac{WebGPU}-based rendering pipeline.
...
\begin{enumerate}[label=\textbf{C\arabic*}]
    ...
    It integrates with the MapLibre style runtime and the \ac{MLT} tile pipeline, and is released as open source.
...
The pipeline composes expression-level rewrites, structural style passes, and a style-driven pruning advisory that feeds \ac{MLT} re-encoding. \Cref{ch:methods} states the joint problem formally under the pixel-equivalence constraint of \cref{eq:visual_equivalence} and details the three levels, with the prior-art positioning and theoretical background in \cref{ch:related_work} and \cref{ch:theoretical_background}.
