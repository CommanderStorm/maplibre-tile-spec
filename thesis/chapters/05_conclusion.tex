...
\begin{enumerate}
    ...
    \item \textbf{Style-driven tile shaving} (\cref{sub_tile_shaving}) produced the cascade documented in \cref{sub_mlt_discussion}: dead elimination narrowed the pruning advisory enough to eliminate entire property columns and tiles from the encoded output.
...
For the geospatial community this means that style authoring and tile encoding can be treated as correlated concerns: the two interact, and tooling that exploits the interaction can recover some transfer and rendering savings without any change to the underlying data or the renderer.
For FPS ($200\%$) and transfer size ($37\%$) this effect is significant, while for load time it is not as significant at $1\%$.
