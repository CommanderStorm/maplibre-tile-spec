...
Vilardaga built \texttt{\mlprop{vt-optimizer}}\footnote{\url{https://github.com/ibesora/vt-optimizer}, last accessed 2025-10-20}, which drops invisible or unused layers and fields in both the style and data.
...
This framing does not appear in prior map-rendering work - neither \texttt{vtshaver} nor \texttt{\mlprop{vt-optimizer}} positions its pruning as query-engine pushdown, and neither tool composes with style-level rewrites to enlarge the pushdown.
...
Stamen Design published \texttt{\mlprop{mapbox-gl-style-remove-defaults}}\footnote{\url{https://github.com/stamen/mapbox-gl-style-remove-defaults}}, a tool that removes properties set to their specification default values from Mapbox GL style JSON.
...
\cref{ch:experiments} quantifies the payoff - \textbf{load times down by up to 75\,\% and tile volume by 28\,\%} on the styles we benchmarked end-to-end - and the OGC API-Tiles standard~\cite{OGCAPITiles} keeps these gains deployable without vendor lock-in.
