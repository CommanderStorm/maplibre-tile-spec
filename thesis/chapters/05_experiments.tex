...
We report the effects on performance generalised in \cref{sub:cross_results} (over the generalisation corpus of \cref{sub:generalisation_corpus}) and the effect of the expression-level steps this work builds upon in \cref{sec:exp_expressions}.
...
Beyond the thesis-wide formal optimisation problem of \cref{sec:problem_statement}, two open problems specific to the data-level pipeline frame this experiment:
...
\noindent In the preceding experiments we evaluated optimisations effectiveness on the curated benchmarking corpus of 15 styles described in \cref{sub:deep_corpus}.
To assess whether the observed gains generalise, we apply the optimiser to the 13~styles of the Maputnik generalisation corpus introduced in \cref{sub:generalisation_corpus}.
...
\noindent In this section we present the full pipeline ablation - all 17 cumulative steps from baseline through tile rewriting, as defined in \cref{sub:ablation} - and analyse the interactions between optimisations categories.
The combined view is essential because the three optimisation categories (expression, structural, data, introduced in \cref{sub:expression_optimisations,sub:structural_optimisations,sub:data_optimisations}) can interact:
...
\emph{Style simplification} (expression passes from \cref{sub:expression_optimisations}) plausibly reduces per-feature evaluation cost: fewer \ac{AST} nodes mean fewer interpreter steps per feature per frame.
\emph{Structural reduction} (dead elimination and layer merging from \cref{sub:structural_optimisations}) plausibly reduces per-frame overhead: fewer layers mean fewer draw calls, fewer GPU state changes, and fewer filter evaluations at the layer level.
\emph{Data reduction} (tile shaving and re-encoding from \cref{sub:data_optimisations}) reduces transfer cost: smaller tiles mean shorter download times and lower decode overhead, primarily improving load time and time to first tile, but because of lower \ac{GC}-spend also average frame times.
