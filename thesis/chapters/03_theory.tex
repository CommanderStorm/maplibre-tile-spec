...
\begin{itemize}
    \item \textbf{Sources} declare where tile data comes from (tile server URL, or inline GeoJSON) and at which zoom levels tiles are available (\mlprop{minzoom}, \mlprop{maxzoom}).
    ...
    \item \textbf{Metadata bounds} (\mlprop{minzoom}, \mlprop{maxzoom}) restrict the zoom range at which a layer is active, allowing the renderer to skip entire layers outside their visible range without evaluating their filters.
    ...
    Tight \mlprop{minzoom}/ \mlprop{maxzoom} bounds therefore eliminate work at two stages of the pipeline before any filter evaluation or geometry decoding occurs.
...
\Cref{lst:ishidden_usage} reproduces the two call-sites that establish those two stages: the worker calls \texttt{layer.isHidden(this.zoom, \mlval{true})} and \texttt{continue}s before bucket creation, and the painter's \texttt{renderLayer} returns immediately on \texttt{layer.isHidden(this.transform.zoom)} before dispatching any \texttt{draw.\mlval{fill}}/ \texttt{draw.\mlval{line}}/ \texttt{draw.\mlval{symbol}} call.
...
In a style context, a comparison such as \texttt{[\mlop{"=="},~2,~3]} can be evaluated at optimizations time and replaced by \mlval{false}.
