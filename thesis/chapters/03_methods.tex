...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[data, right=1.5cm of sopt] (stats) {Tile statistics};

        % Cascade brace — above the inputs, out of the arrows' way
        \draw[decorate, decoration={brace, amplitude=5pt}, thick, gray]
            (sopt.north west) -- (stats.north east)
            node[midway, above=6pt, font=\scriptsize, text=gray, align=center]
            {cascade: fewer live layers $\to$ fewer referenced properties $\to$ smaller tiles};

        % ── Row 2: advisory (centred below inputs) ──
        \node[both] (adv) at ($(sopt.south)!0.5!(stats.south) + (0,-1.4)$)
            {Tile pruning advisory};
        \draw[arrow] (sopt) -- node[left, lbl] {optimised style} (adv);
        \draw[arrow] (stats) -- node[right, lbl] {property/geometry stats} (adv);

        % ── Row 3: MVT → prune → MLT ──
        \node[data, below=1.4cm of adv] (prune) {Prune \& transform};
        \node[data, left=1.4cm of prune] (mvt) {\ac{MVT} tile};
        \node[data, right=1.4cm of prune] (mlt) {\ac{MLT} tile};
...
\begin{sloppypar}
    The Rust encoder replaces prefix matching with -based similarity clustering~\cite{broder1997resemblance}.
