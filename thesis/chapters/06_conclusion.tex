...
The pipeline still produces large overall effects on FPS (\qtyrange{162}{213}{\percent}) and transfer size (\qty{36}{\percent}); it just produces them by stacking independently rather than by interacting.
...
Our rewrite rules (\cref{tab:all_rewrite_rules}) are individually simple, yet their composition through fixpoint iteration achieves globally significant simplification across styles of widely varying complexity (raw reductions of \qtyrange{10}{71}{\percent} across the Maputnik generalization corpus).
