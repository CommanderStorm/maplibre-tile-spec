...
The gap identified in \cref{ch:related_work} is that no existing tool composes style rewrites with data-level pruning into a joint pipeline.
This calls for foundations from several fields.
The map rendering architecture that defines what is being optimised, the compiler techniques whose analogues drive the expression-level passes, the query optimisation concepts that inform filter rewriting and predicate reordering, and the columnar encoding strategies that underpin the data-level passes.

This chapter introduces these foundations in the order in which the optimiser consumes them.
\cref{sec:map_rendering_approaches} describes the map rendering process and its resource pipeline.
\cref{sec:compiler_optimisations} surveys compiler optimisation techniques - peephole rewriting, constant folding, and fixpoint iteration - whose analogues are applied to style expressions.
\cref{sec:query_optimisations} presents query optimisation concepts - predicate pushdown and selectivity estimation - used for filter rewriting and reordering.
\cref{sec:data_encoding} covers the columnar encoding and compression schemes employed at the tile level.
\cref{sec:benchmarking} covers benchmarking methodology.
...
OpenLayers~\cite{OpenLayersWelcome} uses Canvas~2D as its primary renderer, rasterising vector features on the CPU rather than submitting them to the GPU.
...
\item \begin{sloppypar}\textbf{Low-level 3D Graphics APIs} like WebGL~\cite{Getwebglorg}, WebGPU~\cite{WebGPU}, OpenGL~\cite{OpenGL4Reference}, Metal~\cite{incMetalOverview}, Vulkan~\cite{HomeVulkanCross}, DirectX~\cite{DirectXSpecs}, etc.\ for rendering 3D graphics as shown by mapbox~\cite{MapboxMapsNavigation}, maplibre~\cite{maplibreMapLibre}, deck.gl~\cite{HomeDeckgl}, here~\cite{HERETechnologiesWorlds} and many more.
...
While these renderers are powerful, they require data to be preprocessed and optimised specifically for rendering.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        renderer/.style={box, fill=green!10, minimum width=7.5cm, minimum height=1.0cm},
        server/.style={box, fill=blue!12, minimum width=7.5cm, minimum height=0.7cm},
        datasource/.style={box, fill=orange!10, minimum width=5.5cm, minimum height=1.0cm},
        ...
        % Tile Server
        \node[server, below=2.6cm of renderer] (server) {Tile Server};
        ...
        \draw[decorate, decoration={brace, amplitude=6pt}, thick]
            ...
            node[midway, right=10pt, font=\scriptsize\itshape] {over the network};

        % Data Source (bottom)
        \node[datasource, below=1.2cm of server] (datasource) {Data Source};
...
Although all of these resources can be optimised, tiles are usually the most critical due to their continuously streaming nature as a user moves the map.
...
the performance of highly optimised systems such as Microsoft Bing Maps differs significantly from less optimised deployments, such as basemap.de.

The following sections examine each resource in detail, focusing on the performance characteristics and optimisations strategies relevant to this work:
...
Common open-source renderers like maplibre or source available renderers like mapbox do this by shipping plain PNG or \acs{SDF}-PNG~\cite{greenImprovedAlphatestedMagnification2007} sprite sheets together with a lookup table recording where on the sprite sheet each sprite is located, whether it is a \ac{SDF}, and whether there are regions that can be stretched to account for placing text inside the sprite.
...
This observation has direct consequences for any optimiser that rewrites zoom bounds, and is revisited in \cref{ch:methods}.
In particular, tightening a layer's \texttt{maxzoom} is only sound when it accounts for overzoom: removing a layer at zoom~14 would suppress its rendering at zoom~15 and beyond if the source's maximum zoom is~14, because the renderer continues to display overzoomed tiles from the highest available source zoom.
\Cref{lst:overzoom} shows the two operations the slicer performs to synthesise a target tile from the source's \texttt{maxzoom} tile: \textcircled{1}~rescale each coordinate by $2^{\Delta z}$ relative to the target-tile offset (\texttt{offsetX}/\texttt{offsetY}, then \texttt{p.x = p.x * scale - offsetX}), and \textcircled{2}~clip to the target extent with a 128-unit overdraw buffer for continuous edge geometry.
Both operations consume the source's \texttt{maxzoom} tile as input; tightening that \texttt{maxzoom} removes the upstream tile the slicer rescales and clips, which is why the soundness rule above forbids any rewrite that does not preserve every layer's effective overzoom range.

\begin{lstlisting}[
  language=TypeScript,
  caption={Condensed MapLibre GL-JS overzoom implementation (\texttt{src/source/vector\_tile\_overzoomed.ts:87-125})},
  label=lst:overzoom,
  escapeinside={(*@}{@*)},
]
function sliceVectorTileLayer(sourceLayer, maxZoomTileID, targetTileID) {
    ...
    for (const feature of sourceLayer) {
        ...
        // (*@\textcircled{1}@*) Rescale each coordinate into the target tile's space
        for (const ring of geometry) for (const p of ring) {
            p.x = p.x * scale - offsetX;
            p.y = p.y * scale - offsetY;
        }
        // (*@\textcircled{2}@*) Clip to the target extent with a 128-unit overdraw buffer
...
No symmetric \enquote{underzoom} behaviour exists:
If a source has no tile at a zoom lower than \texttt{minzoom}, the layer is  not rendered there.
...
\begin{itemize}
    \item \textbf{Raster tiles} deliver pre-rendered images (PNG, JPEG, WebP, Avif, JPEGXL) rather than vector geometry.
    ...
    \item \textbf{Analysis-focused formats} such as FlatGeobuf~\cite{flatgeobuf2018}, GeoParquet~\cite{geoparquetSpec2024}, and GeoArrow are optimised for analytical queries (filtering, aggregation, joins) rather than tile-based rendering.
    ...
    GeoParquet in particular supports row-group-level predicate pushdown and column pruning - operations analogous to the tile shaving and selective column access this thesis applies to \ac{MLT} - but operates on large per-file datasets rather than on the small, spatially-partitioned tiles that interactive renderers fetch individually; the per-tile overhead of Parquet's row-group and column-chunk metadata would dominate at typical tile sizes (\qtyrange{10}{100}{\kilo\byte}).

    \item \textbf{GeoJSON}~\cite{butlerGeoJSONFormat2016} is a JSON-based geospatial interchange format.
    ...
    \item \textbf{\acf{MVT}}~\cite{Vectortilespec21Master} is the dominant vector tile format, adopted as a \emph{de~facto} standard across open-source map renderers including MapLibre.
    It uses \acp{PBF}~\cite{ProtocolBuffers} as its wire format and stores features in a \emph{row-oriented} layout:
\begin{itemize}
    ...
    This is also known as "turtle style graphics"\footnote{\url{https://docs.python.org/3/library/turtle.html}} as an analogy to the \texttt{turtle} python module.
...
Beyond the choice of tile format, a number of transfer-layer optimisations can improve delivery performance:
utilising the approximately \qty{14}{\kilo\byte} TCP slow-start window as discussed by Nathaniel~\cite{WhyYourWebsite}, edge caching~\cite{BestPracticesMap}, HTTP/2 multiplexing~\cite{belsheHTTP2RFC2015} (and its QUIC-based successor HTTP/3~\cite{bishopHTTP3RFC2022} for lossy mobile networks), and modern content-encoding algorithms such as Zstandard~\cite{colletZstandardRFC2021} and Brotli~\cite{alakuijalaBrotliRFC2016} that achieve higher compression ratios than gzip's LZ77 foundation~\cite{zivUniversalAlgorithmSequential1977}.
Tile archive formats such as PMTiles~\cite{liuPMTiles2022} and COMTiles~\cite{tremmelCOMTilesCloudOptimized2023} address the container layer by enabling serverless, cloud-native tile delivery from a single file in blob storage.
Since \ac{MVT} is the established baseline and \ac{MLT} aims to improve upon it, this thesis evaluates both formats and uses the transition from \ac{MVT} to \ac{MLT} as one of its data-level optimisations strategies.
...
\begin{itemize}
    ...
    \item \textbf{Callback-based} styling, as proposed by Gleo~\cite{ortegaGleoWebGLMap} or Navara~\cite{Navara}, replace declarative rules with imperative callbacks that the application registers with the renderer.
    This offers maximum flexibility but makes static analysis and automatic optimisations impossible, since the styling logic is opaque to the rendering engine.
\end{itemize}

This thesis focuses on client-side declarative styles, specifically the MapLibre style specification, as it is the most widely adopted approach in open-source map rendering and - its declarative nature makes it amenable to static analysis and automatic optimisations.
...
\begin{itemize}
    \item \textbf{Sources} declare where tile data comes from (tile server URL, or inline GeoJSON) and at which zoom levels tiles are available (\texttt{minzoom}, \texttt{maxzoom}).

    \item \textbf{Layers} are the central rendering unit.
    Each layer references a source and a \emph{source-layer} within that source, specifies a \emph{type} (\texttt{fill}, \texttt{line}, \texttt{circle}, \texttt{symbol}, \texttt{fill-extrusion}, \texttt{raster}, \texttt{background}, or \texttt{hillshade}) and carries a set of paint and layout properties that control its appearance.
...
\begin{lstlisting}[language=TypeScript,caption={MapLibre GL-JS usages of \texttt{isHidden}},label=lst:ishidden_usage]
...
The resulting expression trees can be deeply nested and may contain redundant or unreachable branches - characteristics that make them amenable to the same kinds of simplification and optimisations techniques used in compilers and database query planners.

\Cref{lst:worker_pipeline} shows the worker's tile-parse pipeline as five named steps, marked inline as
\textcircled{a}--\textcircled{e}.

Every optimisation in \cref{ch:methods} attaches to one of these steps:
Merging layers reduces work for \textcircled{a}.
Pruning features server-side makes \textcircled{b} not handle them.
Tightening \texttt{minzoom}/\texttt{maxzoom} makes the
\textcircled{c} guard true earlier.
Pre-folding zoom-dependent constants reduces the work in \textcircled{d} while
...
\begin{lstlisting}[
    language=TypeScript,
    escapeinside={(*@}{@*)},
    caption={MapLibre GL-JS WorkerTile.parse processing pipeline (\texttt{src/source/worker\_tile.ts:85-127})},
    label=lst:worker_pipeline
]
// (*@\textcircled{a}@*) Iterate source layers
...
for (const sourceLayerId in layerFamilies) {
    ...
    // (*@\textcircled{b}@*) Collect raw features
    const features = [];
    ...
    for (const family of layerFamilies[sourceLayerId]) {
        ...
        // (*@\textcircled{c}@*) Skip hidden layers
        if (layer.isHidden(this.zoom, true)) continue;

        // (*@\textcircled{d}@*) Recalculate zoom-dependent expressions
        recalculateLayers(family, this.zoom, availableImages);

        const bucket = buckets[layer.id] = layer.createBucket({...});

        // (*@\textcircled{e}@*) Filter features and build vertex data
...
\section{Compiler Optimisation Techniques}\label{sec:compiler_optimisations}

The style expressions used by map renderers such as MapLibre are, in essence, small programs:
they contain conditionals (\texttt{case}, \texttt{match}), boolean operators (\texttt{all}, \texttt{any}), arithmetic, and data access (\texttt{get}, \texttt{has}), variables (global state, local state, feature state).
...
This section introduces the foundational concepts applied in \cref{ch:methods}.

\subsection{Peephole Optimisation}

\begin{sloppypar}
A \emph{peephole optimiser} examines a small, fixed-size window of code (the \enquote{peephole}) and applies local pattern-matching rewrites that replace instruction sequences with shorter or faster equivalents~\cite{davidsonCodeSelectionObject1984}.
...
The simplicity of individual rules is offset by their composition through repeated application - many small, local improvements accumulate into significant global optimisations.
\end{sloppypar}
...
In a style context, a comparison such as \texttt{["==",~2,~3]} can be evaluated at optimisations time and replaced by \texttt{false}.
...
Individual peephole rules may expose further optimisations opportunities:
...
The expression-level optimiser is, formally, a \ac{TRS}: the style expressions are terms, and the peephole rules are rewrite rules applied modulo a traversal strategy~\cite{baaderTermRewritingAll1998}.
...
The standard \ac{TRS} notions of confluence (whether all rule-application sequences converge to the same normal form) and termination (whether all sequences reach a normal form) are revisited in the context of the concrete rewrite system in \cref{sub:expression_optimisations}.
A closely parallel case study is Halide's \ac{TRS}: Newcomb et~al.~\cite{newcombVerifyingImprovingHalide2020} showed that Halide's \ac{DSL} optimiser - which, like this thesis's expression passes, applies a set of hand-written rewrite rules in a fixed order to simplify \ac{DSL} expression trees - suffers from the same confluence and termination concerns, and developed synthesis-based techniques to verify correctness and discover missing rules.
\end{sloppypar}


\subsection{Profile-Guided Optimisation}\label{sub:pgo}
...
Classical PGO instruments a program, collects execution frequencies and branch probabilities from representative inputs, and feeds this profile back into the compiler to guide inlining, code layout, and branch prediction hints.
Arnold et~al.~\cite{arnoldSurveyAdaptiveOptimization2005} survey how virtual machines extend \ac{PGO} to an \emph{adaptive} setting, where profiles are collected and optimisations applied continuously at runtime.

The style optimiser performs an analogous form of \ac{PGO}:
tile-level statistics (value cardinalities, per-property frequency distributions, geometry-type counts) are the ``profile data,'' and the stats-driven folding pass uses this profile to specialise the style for the observed data distribution - collapsing \texttt{match} arms whose values never occur in the tile census, tightening zoom bounds based on observed data ranges, and converting high-selectivity predicates into constants.
The key difference from classical \ac{PGO} is that the profile is exact (the tile census covers every feature in the tileset at a sample rate of~1.0) rather than sampled, and the optimisation target is a declarative style document rather than machine code.

\section{Query Optimisation}\label{sec:query_optimisations}
...
Techniques from relational query optimisations therefore apply directly.

\subsection{Cost-Based Query Optimisation}

A \emph{cost-based query optimiser} selects among semantically equivalent query execution plans by estimating the cost of each plan and choosing the cheapest~\cite{selingerAccessPathSelection1979}.
...
A joint-distribution estimator, such as the one proposed in \cref{sec:seletivity_reordering} would therefore give tighter bounds, but the selectivity signal used in this thesis is consumed only by a \emph{reordering} heuristic - not by a cost-based plan chooser - and reordering is monotone in the ordering of the individual selectivities rather than in their absolute values.
...
\subsection{Materialized Views}\label{sub:materialized_views}

A \emph{materialized view} is a derived relation whose contents are precomputed from a query over base tables and stored for fast access~\cite{guptaMaintenanceMaterializedViews1995}.
When the base data changes, the materialized view must be updated - either by full recomputation or by \emph{incremental view maintenance}, which propagates only the delta.
The cascade developed in this thesis has a direct structural analogue: 
the tile set is the base relation, the rendering style is the view definition, and the shaved tile is the materialized view.
When the style changes (e.g., a layer is removed by dead elimination), the pruning advisory changes, which in turn changes the shaved output - precisely the IVM problem of maintaining a derived dataset under view-definition changes.

The future direction of serving-time applicability (\cref{sec:conclusion_future_work}) - applying the advisory as a per-request filter rather than a batch operation - is a form of virtual (non-materialized) view evaluation.
...
The style optimiser performs partial evaluation of the rendering function: the style (static input) is known at optimisation time, while tile data (dynamic input) varies at runtime.
Filter-to-property constant propagation (\cref{sub:structural_optimisations}) is a textbook instance:
...
The encoding strategy competition (\cref{sub:encoding_strategies}) shares the spirit of \emph{superoptimization}~\cite{massalinSuperoptimizerLookingShortest1987} in that it exhaustively evaluates all candidate strategies over the actual data and retains the smallest result, rather than using profile data to guide a heuristic decision as in classical \ac{PGO} (\cref{sub:pgo}).
Unlike true superoptimization, however, the candidate set is designer-specified (sort orders $\times$ encoding schemes) and the profiler prunes the search space before exhaustive evaluation; it is therefore more precisely characterised as \emph{exhaustive strategy selection} over a bounded candidate space.
...
This section surveys the encoding and compression techniques used in the data-level optimisations of \cref{ch:methods}.
...
The encoding competition in \cref{sub:encoding_strategies} takes an exhaustive rather than cost-model approach, trialling a pruned set of candidate encodings and retaining the smallest output.
...
\begin{itemize}
    ...
    \item \textbf{\acf{FSST}}~\cite{bonczFSSTFastRandom2020} is a lightweight, block-oriented string compressor that learns a \emph{symbol table} of up to 255 frequent byte sequences from the input corpus and replaces occurrences with single-byte codes.
    ...
    \ac{FSST} and dictionary encoding can be combined.
    This means that applying \ac{FSST} to the deduplicated dictionary strings compresses both the unique values and the byte-level redundancy within them.
\end{itemize}
...
For tile data, features are assigned a curve index based on their first vertex coordinate, and the feature ordering that produces the smallest encoded output (across Morton, Hilbert, and non-spatial orderings) is selected by the encoding competition described in \cref{ch:methods}.
...
The estimate is unbiased with variance $O(1/k)$~\cite{broderResemblanceContainmentDocuments1998}, so the accuracy is fine-tunable.
...
The purpose of benchmarking is to evaluate system behaviour, compare alternative implementations, and quantify the effects of optimisations.

According to Jain~\cite{jainArtComputerSystems1991}, a good evaluation must start with selecting relevant metrics.
Georges et~al.~\cite{georgesStatisticallyRigorousJava2007} later demonstrated that na\"ive averaging of execution times on managed runtimes (such as JavaScript engines) leads to unreliable conclusions, advocating instead for confidence intervals computed over multiple independent JVM/engine invocations - a principle that informs the bootstrap CI methodology used in this thesis.
The bootstrap itself - a resampling method for constructing confidence intervals without distributional assumptions - was formalised by Efron and Tibshirani~\cite{efronIntroductionBootstrap1993} and is the specific statistical technique applied throughout this thesis's evaluation (\cref{ch:experiments}).
Hoefler and Belli~\cite{hoeflerScientificBenchmarkingParallel2015} codified twelve common pitfalls in reporting performance results; the evaluation in \cref{ch:experiments} follows their guidelines by reporting per-metric confidence intervals and separating throughput from latency measurements.
...
\begin{itemize}
    \item \textbf{Resource utilisation}, such as CPU load, GPU utilisation, and memory consumption to have pointers where further optimisations is possible.
    ...
    \item \textbf{Power consumption} in Watts, which is relevant for mobile devices and battery-powered systems.  This thesis does not measure power consumption - no RAPL counters, battery deltas, or external power meters are used, as the benchmark host is a desktop workstation rather than a battery-constrained mobile device.  Power is listed here as a candidate metric because transfer-size reductions and reduced draw-call counts are established proxies for energy savings on mobile targets. Cellular radio energy is proportional to data volume, and GPU active time scales with draw-call count.
\end{itemize}
...
With these theoretical tools in place, the mapping from theory to the optimiser's three-level pipeline can be made explicit.

The compiler techniques surveyed above - peephole rewriting, constant folding, and fixpoint iteration - supply the rewrite strategies for the expression-level passes that simplify individual style expressions without schema knowledge.
The query-optimisation concepts - predicate pushdown and selectivity estimation - inform the structural passes and, critically, the pruning advisory that bridges style and data: the advisory is a pushdown plan derived from the optimised style, analogous to a query planner projecting only the columns a query reads.
The encoding techniques - columnar storage, integer coding, \ac{FSST}, and space-filling curves - underpin the data-level passes that re-encode shaved tiles with automatic strategy selection.

The following chapter formalises the optimisation problem and applies these foundations to the concrete design and implementation of the optimiser.
