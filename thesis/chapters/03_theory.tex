...
\begin{itemize}
    ...
    The worker invokes \texttt{layer.isHidden(this.zoom, \mlval{true})} to skip bucket creation during tile parsing, while the renderer returns early from \texttt{renderLayer} on \texttt{layer.isHidden(this.transform.zoom)} before dispatching draw calls.
