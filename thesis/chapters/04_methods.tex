...
Under the soundness contract of \cref{sec:scope}, the passes from~\cref{sub:pipeline} together aim to improve the deployment metrics of \cref{sub:metrics} in exchange for the preprocessing cost $C_\text{total}$ of \cref{sub:preprocessing_cost_model}.
...
Around it are three supporting components, each detailed in its own section.
A code-generation front end (\cref{sub:codegen}) produces typed Rust types from the upstream MapLibre style specification.
...
Second, it \emph{normalizes} the result into a stable intermediate representation (the \texttt{MIR}) that resolves cross-cutting concerns.
...
A na\"ive stats-driven tightening - \enquote{the last tile containing polygons appears at zoom~12, therefore set \mlprop{maxzoom}=\mlval{12}} - would change the rendered output if the layer is visible at zoom~13 or~14 via overzoom from zoom~12.
...
\bottomrule
\end{tblr}
\caption{Data-level optimization techniques, all requiring a full tile scan. Together they form a logical, physical, and cost pipeline: tile shaving eliminates unreferenced data, re-encoding transforms the wire format, and compression optimizations select the most compact per-column encoding strategy.}
...
The encoder chooses \texttt{Id32} when the maximum observed ID fits in \texttt{u32}, narrowing 64-bit-declared ID columns whose actual values fit in 32 bits.
...
Warm-start caching makes multi-trial encoding affordable.
The encoder computes \ac{FSST} compressors, Morton metadata, and vertex-strategy decisions once and reuses them across sort trials, and scratch buffers grow on first use and are recycled across candidates.
A scoped rollback mechanism shares the encoder's buffers across candidates, so the inner per-stream competition resolves before the enclosing sort-strategy trial commits.
