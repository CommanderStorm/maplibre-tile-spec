...
Metrics reported include load time, frames per second, frame time percentiles, \ac{AST} complexity (node count, maximum depth), and style size (raw, gzip, Brotli).
...
Confirming the design expectation (\cref{sub_pipeline}), all styles converged within two to three fixpoint iterations; no style in the benchmark or generalisation corpus reached the hard cap of eight iterations.
...
The alternative would be to have an more complex structural pass.
\end{sloppypar}
...
Each of these style-level changes also narrows the set of tile data that the rendering pipeline references - an effect that the preceding discussions identified as an interaction from style optimisation through the tile pruning advisory into smaller tiles.
This experiment asks: can the \emph{data} itself be optimised using style information, and to what extent do style-level and data-level optimisations interact?
...
The central question is whether style-level and data-level optimisations interact - that is, whether optimising the style first and then encoding the resulting tile set produces measurably different results than either optimisation category in isolation, and which level contributes the dominant share.
...
After shaving determines what data survives, the encoding stage determines whether the shaving savings reach the wire.
...
The multi-strategy competition is not an independent contribution but the \emph{delivery mechanism} for the shaved data.
...
\emph{Tile pruning and MVT re-encoding} ($C_\text{enc}$, MVT) takes \qty{15.6}{\second} using Rayon parallelism.
\emph{\ac{MLT} encoding with sort-strategy competition} ($C_\text{enc}$, MLT auto) is the dominant term: \qty{5}{\minute}\,\qty{42}{\second} single-threaded, or \qty{47}{\second} with 16 threads.
Without the competition ($C_\text{enc}$, MLT none), single-threaded encoding takes \qty{2}{\minute}\,\qty{55}{\second}.
...
$C_\text{enc}$ (MVT prune) & Tile pruning + MVT re-encode (16 threads) & \qty{15.6}{\second} \\
$C_\text{enc}$ (MLT none, 1T) & MLT encoding, single strategy, 1 thread & \qty{175}{\second} \\
$C_\text{enc}$ (MLT auto, 1T) & MLT encoding, full competition, 1 thread & \qty{342}{\second} \\
$C_\text{enc}$ (MLT auto, 16T) & MLT encoding, full competition, 16 threads & \qty{46.7}{\second} \\
\midrule
...
The total single-threaded pipeline time (statistics collection through MLT encoding with full competition) is under \qty{7}{\minute} on the Germany tileset; with 16-thread parallelism the end-to-end wall-clock time drops to approximately \qty{2}{\minute} --- perfectly acceptable for an offline build stage.
...
Their contributions are additive: the aggregate data shows no super-additive interaction at the median level, though style-level dead layer elimination does narrow the tile pruning advisory, providing a modest assist to tile shaving.
...
For each of the 15 benchmark styles, the baseline and optimised renderings were compared at integer zoom levels 0-14 across all 18 scenarios, with both axis-aligned (no pitch, no bearing) and a fixed pitched view, all at a \texttt{pixelRatio} of~1 (standard display density).
