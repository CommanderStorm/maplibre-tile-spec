...
We modelled this architecture on peephole optimisers in production compilers: individual rules are simple and local, but their composition through fixpoint iteration achieves globally significant simplification.
...
\paragraph{Selectivity reordering}\label{sec:seletivity_reordering}
For variadic boolean expressions (\texttt{all}, \texttt{any}), the pass reorders predicates by estimated selectivity to exploit short-circuit evaluation.
...
The preceding style optimisations thus feed into data-level savings, though the aggregate interaction is additive rather than synergistic (\cref{sub:mlt_results}).
...
We reformulate the encoding problem in the Rust-based encoder as a \emph{multi-strategy competition} inspired by \ac{PGO} in compilers~\cite{changUsingProfileInformation1991} and the instance-optimised systems vision of SageDB~\cite{dingSageDBInstanceOptimizedData2022}:
...
\begin{enumerate}
    ...
    \item \textbf{Spatial (Morton/Z-order)}: the encoder sorts features by the Z-order curve index of their first vertex, computed via bit-interleaving.  Fast to compute; provides reasonable spatial locality.
...
\begin{sloppypar}
    Our Rust encoder replaces prefix matching with -based similarity clustering~\cite{broderResemblanceContainmentDocuments1998}.
...
Our evaluation considers several dimensions:
