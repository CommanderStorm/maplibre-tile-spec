...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Loop-back from update
        \draw[arrow] (upd.west) -- ++(-2.4,0) |- (more.west);

        % No: discard, loop back

        % Exit loop
        \node[output, right=2.2cm of more] (best)
        {Emit smallest encoding\\%
            {\scriptsize\texttt{best.into\_layer\_bytes()}}};

        % no's
        \draw[arrow] (cmp.east) -- ++(2.0,0)
        node[above, label, pos=0.4] {noop}
        |- (more.east);
        \draw[arrow] (more.south) ++(0,0) -- ++(3.4,0)
            node[above, label, pos=0.15] {no}
            |- (best.east);
    \end{tikzpicture}
