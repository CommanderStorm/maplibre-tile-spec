...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % tanlged arrows
        \draw[query] (corpus) -- node[below, label] {validate} (serial);
        \draw[query] (opt) -- node[right, label] {optimise} (serial);
        \draw[query] (scan) -- node[above, near start, label] {tile\\statistics} (opt);
        \draw[arrow] (serial) -- node[right, label] {sample styles} (test);
