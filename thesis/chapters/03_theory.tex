...
\Cref{lst:overzoom} shows the two operations the slicer performs to synthesise a target tile from the source's \texttt{maxzoom} tile: \stepcirc{1}~rescale each coordinate by $2^{\Delta z}$ relative to the target-tile offset (\texttt{offsetX}/\texttt{offsetY}, then \texttt{p.x = p.x * scale - offsetX}), and \stepcirc{2}~clip to the target extent with a 128-unit overdraw buffer for continuous edge geometry.
...
\stepcirc{a}--\stepcirc{e}.

Every optimisation in \cref{ch:methods} attaches to one of these steps:
Merging layers reduces work for \stepcirc{a}.
Pruning features server-side makes \stepcirc{b} not handle them.
Tightening \texttt{minzoom}/\texttt{maxzoom} makes the
\stepcirc{c} guard true earlier.
Pre-folding zoom-dependent constants reduces the work in \stepcirc{d} while
...
// (*@\stepcirc{a}@*) Iterate source layers
...
for (const sourceLayerId in layerFamilies) {
    ...
    // (*@\stepcirc{b}@*) Collect raw features
    const features = [];
    ...
    for (const family of layerFamilies[sourceLayerId]) {
        ...
        // (*@\stepcirc{c}@*) Skip hidden layers
        if (layer.isHidden(this.zoom, true)) continue;

        // (*@\stepcirc{d}@*) Recalculate zoom-dependent expressions
        recalculateLayers(family, this.zoom, availableImages);

        const bucket = buckets[layer.id] = layer.createBucket({...});

        // (*@\stepcirc{e}@*) Filter features and build vertex data
