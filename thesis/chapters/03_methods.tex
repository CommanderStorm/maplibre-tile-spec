...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % tanlged arrows
        \draw[test] (corpus) -- node[red,below, label] {validate} (serial);
        \draw[query] (opt) -- node[right, label] {optimise} (serial);
        \draw[query] (scan) -- node[above, near start, label] {statistics} (opt);
        \draw[test] (serial) -- node[right, label] {validate} (test);
        \draw[test] (test) -- node[red,right, label] {validate} (opt);
    \end{tikzpicture}
