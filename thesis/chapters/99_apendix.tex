...
\cref{tab:all_rewrite_rules} lists the complete set of 47 expression rewrite rules implemented in the optimiser, grouped by pass.
Each rule is characterised by its pass assignment, scope (filter-only or both filters and paint/layout properties), type (pure, MIR-aware, or stats-driven), and a brief description of the pattern it matches and the rewrite it produces.
...
39 & Simplify & both & pure & Rewrite \texttt{case} to \texttt{match} for $O(1)$ dispatch when all arms test equality on the same expression \\
...
\section{Per-Style Ablation Results}\label{app_per_style}

This section provides the full per-style, per-ablation-step results for each of the benchmark styles.
...
Steps 16--19 (selectivity reorder, tile shaving, tile rewrite) require tile statistics and are only available for fiord and liberty, which were benchmarked with an \texttt{.mbtiles} source.
Americana is excluded from the per-style tables due to incomplete benchmark data (only 3 of 20 ablation steps completed).
...
\cref{tab:tile_sizes_per_zoom} shows the per-zoom-level tile size (gzip) for the Germany OpenMapTiles tileset across four encoding configurations: the \ac{MVT} baseline, the Tremmel \& Zink \ac{MLT}-Java encoder, the \ac{MLT}-Rust encoder (without shaving), and the full pipeline (style optimizations + tile shaving + \ac{MLT}-Rust encoding).
The shaved configuration uses the advisory generated from the fiord style; other styles would produce slightly different shaving results depending on their property and layer references.
...
shaving + MLT encoding) relative to the MVT baseline}
...
\cref{tab:maputnik_full} lists the 13 styles from the Maputnik editor catalogue used in the cross-style generalisation study (\cref{sec_exp_cross_style}), together with per-style metrics.
Two catalogue entries were unavailable at the time of measurement (HTTP~403).
The optimiser was run with all static passes enabled (no tile statistics).
