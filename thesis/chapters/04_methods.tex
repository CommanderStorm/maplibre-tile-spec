...
The pass treats a layer as dead - and removes it - under three conditions.
First, its filter is the literal \mlval{false}, produced by contradiction detection or stats-driven folding in the expression phase.
Second, its source layer contains zero features according to tile statistics.
Third, its geometry type does not match the source layer's geometry types, for example a \texttt{\mlval{fill}} layer targeting a source layer that contains only points.
...
\textbf{Coverage.}
Every feature drawn by the original $k$ layers must still be drawn by the merged layer, with the same paint and layout properties it would have had.
This holds because the merged filter is the disjunction of the original filters ($\bigvee_i F_i$), the merged paint/layout expression is a \mlop{case}/ \mlop{match} on the same discriminator that selects the original per-layer value, and the optimizer refuses the merge whenever any differing property lacks \texttt{expression.feature} support (so there is always a syntactic vehicle for the per-feature branch).

\textbf{Painter order.}
The per-layer draw order (the painter's algorithm) must be preserved so that the top-most pixel at each screen position is unchanged.
The synthesised sort key delivers this: for layer types that expose a \texttt{\{\mlprop{fill,line,circle}\}\mlprop{-sort-key}} layout property, the pass emits a \mlop{case}/ \mlop{match} sort key that maps each original layer's features to an ascending integer reflecting the original declaration order, and the MapLibre renderer sorts features within the merged layer by this key before drawing.
...
First, the encoder converts the row-oriented \ac{MVT} features into an owned columnar representation (\texttt{StagedLayer01}), with separate arrays for each property column, the geometry vertex buffer, and the feature ID column.
...
\begin{sloppypar}
    ...
    An in-tree proptest suite covers every \texttt{IntEncoder} variant with round-trip assertions across the full representable range of the selected physical type.
    A libFuzzer harness fuzzes full \texttt{DecodedLayer} round-trips, with a corpus accumulated over previous runs.
    Snapshot tests via \texttt{insta} pin the per-stream profiler's candidate lists to known-good outputs.
    The pixel-exact visual validation described in \cref{sec:visual_correctness} exercises the full decode path against a known-good baseline.
\end{sloppypar}
