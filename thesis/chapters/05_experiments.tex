...
\begin{tcolorbox}[
  enhanced,
  blanker,
  borderline west={1mm}{0pt}{TUMOrange!50},
  left=5mm,
  right=0mm,
  top=1mm,
  bottom=1mm,
...
A handful of styles from the pre-screening set do not appear in the numbers below.
\emph{BasemapDE Colour} and \emph{BasemapDE Topo} were not benchmarked within the project's time budget.
\emph{ICGC Fosc} and \emph{ICGC Gris} rely on legacy style properties that MapLibre silently migrates in a way that changes rendering, so a fair before/after comparison was not possible.
\emph{Americana} triggered an \ac{MLT} rendering fault at certain viewports during the cross-style run.
All reported aggregates and per-style tables refer to the post-exclusion corpus.
\end{tcolorbox}
...
\centering
    ...
    \end{subfigure}
\caption[Encoder comparison: per-zoom breakdown and total tileset size]{Encoder comparison for the Germany OpenMapTiles tileset (zoom~0-14, \num{256588} tiles). MLT-Rust (this work) achieves \qty{28.3}{\percent} smaller total size than the MLT-Java baseline under no wire compression; the Java encoder drifts above \ac{MVT} size at several zoom levels under Brotli/zstd, whereas the Rust encoder remains below \ac{MVT} throughout.}
...
\Cref{tab:preprocessing_cost} reports wall-clock time for each stage of the preprocessing pipeline on the Germany OpenMapTiles tileset (\num{256156} tiles after pruning, zoom 0-14), which we measured on an AMD Ryzen~7 3700X (8 cores / 16 threads), \qty{46}{\gibi\byte} RAM, NVMe SSD; median of three runs.
...
We conducted all rendering benchmarks on a single hardware configuration (GPU model, CPU, memory) running a specific browser version with ANGLE/Vulkan as the graphics backend; the full host, driver, toolchain, and dependency dump is in \cref{app:bench_env}.
...
Composed end-to-end on Fiord and Liberty, the full pipeline reduces load time by \qtyrange{67}{71}{\percent} and improves \ac{FPS} by \qtyrange{148}{150}{\percent}, but the cross-family interaction is \textbf{additive rather than synergistic}.
